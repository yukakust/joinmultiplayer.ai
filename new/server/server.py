...
class ApplicationHandler(SimpleHTTPRequestHandler):
    ...
    def handle_get(self) -> None:
        ...
        if path.startswith("/api/public/"):
            ...
            return
        if path.rstrip("/") in SPA_ROUTES or self.spa_fallback(path):
    ...
    def spa_fallback(self, path: str) -> bool:
        """Serve the SPA shell for extension-less paths that have no file.

        The production edge rewrites unknown routes (/d04, /map/, /record/…)
        to index.html; this fallback reproduces that behavior so the copy is
        self-contained behind any plain reverse proxy.
        """
