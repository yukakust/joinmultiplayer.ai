...
BUILD_DIR="$(mktemp -d "$DOWNLOADS_DIR/pocket-i-checkpoint7p-build.XXXXXX")"
...
echo "Building Pocket i with the accepted whole-turn harness from: $SHORT_REVISION"
...
npm ci
npm test
echo "Preparing the local DeBERTa support checker and llama.cpp runtime..."
...
for packed_module in audit-store.cjs reranker.cjs secret-scan.cjs; do
...
APP_ROOT="$(dirname "$(dirname "$ASAR_PATH")")"
if [ ! -x "$APP_ROOT/Resources/runtime/llama-server" ]; then
  echo "The packaged application is missing executable llama-server." >&2
  exit 1
fi
echo "PACKAGED_WHOLE_TURN_HARNESS_OK"
...
if [ "$APP_VERSION" != "0.1.0-alpha.18" ]; then
