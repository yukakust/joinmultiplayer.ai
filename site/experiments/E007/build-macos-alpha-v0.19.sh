...
EXPECTED_REVISION="59569a45a3dbf626b18e582b6ebad9519f8635ef"
SHORT_REVISION="59569a4"
...
BUILD_DIR="$(mktemp -d "$DOWNLOADS_DIR/pocket-i-checkpoint7s3-build.XXXXXX")"
...
echo "Building Pocket i with two-stage grounded extraction from: $SHORT_REVISION"
...
npm ci
npm test
echo "Preparing local memory checks and the thin desktop runtime..."
...
for packed_module in audit-store.cjs evidence.cjs reranker.cjs remote-inference.cjs secret-scan.cjs; do
...
NODE
echo "PACKAGED_YUKABOX_BRAIN_OK"
...
if [ "$APP_VERSION" != "0.1.0-alpha.21" ]; then
