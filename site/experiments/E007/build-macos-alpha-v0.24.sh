...
EXPECTED_REVISION="b8bdb5b2cce0dc3e567460db2f2e99772406511d"
SHORT_REVISION="b8bdb5b"
...
BUILD_DIR="$(mktemp -d "$DOWNLOADS_DIR/pocket-i-checkpoint7u2-build.XXXXXX")"
...
echo "Building Pocket i with the consented server path selected by default from: $SHORT_REVISION"
...
if (!renderer.includes("remoteConsentDismissed")) process.exit(6);
NODE
echo "PACKAGED_DEFAULT_SERVER_WITH_CONSENT_OK"

APP_VERSION="$(node -p "require('$REPO_DIR/desktop/app/package.json').version")"
if [ "$APP_VERSION" != "0.1.0-alpha.26" ]; then
