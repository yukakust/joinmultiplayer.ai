...
EXPECTED_REVISION="e33cd4ff426617fd6f170d9e8d79de91b7ce4d45"
SHORT_REVISION="e33cd4f"
...
BUILD_DIR="$(mktemp -d "$DOWNLOADS_DIR/pocket-i-checkpoint7u-build.XXXXXX")"
...
echo "Building Pocket i with explicit remote-brain consent from: $SHORT_REVISION"
...
const main = fs.readFileSync("$REPO_DIR/desktop/app/main.cjs", "utf8");
...
if (!setup.includes("remote-brain-consent.json")) process.exit(5);
if (!main.includes("requireRemoteConsent")) process.exit(6);
NODE
echo "PACKAGED_EXPLICIT_REMOTE_CONSENT_OK"

APP_VERSION="$(node -p "require('$REPO_DIR/desktop/app/package.json').version")"
if [ "$APP_VERSION" != "0.1.0-alpha.25" ]; then
