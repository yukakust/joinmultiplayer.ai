...
EXPECTED_REVISION="a47048bcefc3c7621c24c319d7854e100db7e8ea"
...
RUN_DIR="$(mktemp -d "$HOME/Downloads/pocket-i-anchor-ab.XXXXXX")"
cleanup() {
  if [ -d "$RUN_DIR" ]; then
    rm -rf -- "$RUN_DIR"
  fi
...
"$PYTHON_PATH" \
  "$RUN_DIR/repo/site/experiments/E007/anchor-search-ab-v0.1.py" \
