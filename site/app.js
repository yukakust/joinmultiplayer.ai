...
async function loadE005Gate5C() {
  ...
  if (!target) return;
  try {
    const response = await fetch("/experiments/E005/gate-5c-design-v0.1.json", { cache: "no-store" });
    if (!response.ok) throw new Error("E005 Gate 5C design unavailable");
    const data = await response.json();
    const pick = value => escapeHTML(value?.[language] || value?.en || "");
