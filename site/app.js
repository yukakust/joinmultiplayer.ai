...
async function loadE005Gate5B2() {
  ...
  if (!target) return;
  try {
    const [response, correctedResponse, rubricV3Response, rubricV4Response, preflightResponse, calibrationResponse, calibrationV2Response, calibrationV3Response, phiLoadResponse, qwen8ResultResponse] = await Promise.all([
    ...
    if (!response.ok || !correctedResponse.ok || !rubricV3Response.ok || !rubricV4Response.ok || !preflightResponse.ok || !calibrationResponse.ok || !calibrationV2Response.ok || !calibrationV3Response.ok || !phiLoadResponse.ok || !qwen8ResultResponse.ok) throw new Error("E005 Gate 5B.2 checkpoint unavailable");
    const data = await response.json();
    ...
    const calibrationMarkup = `<section class="e005-gate4-result-verdict is-failed"><span>${localized({ en: "JUDGE A · 8B REJECTED", ru: "СУДЬЯ A · 8B ОТКЛОНЁН" })}</span><h2>${qwen8Result.score} / ${qwen8Result.required}</h2><p>${pick(qwen8Result.plain_result)}</p></section><section class="e005-gate4-lessons-status"><strong>${localized({ en: "JUDGE B · 14B IS NEXT", ru: "СЛЕДУЮЩИЙ — СУДЬЯ B · 14B" })}</strong><p>${localized({ en: "It receives the same ruler and still sees zero E005 answers before 12/12.", ru: "Он получает ту же линейку и не увидит ответы E005 до результата 12/12." })}</p></section><section class="e005-gate4-lessons-status"><strong>${localized({ en: "ZERO EXPERIMENT ANSWERS SEEN", ru: "УВИДЕНО 0 ОТВЕТОВ ЭКСПЕРИМЕНТА" })}</strong><p>${localized({ en: rubricV4.blindness, ru: "Судьи не знают модель и вариант, создавшие ответ." })}</p></section><details class="e005-all-controls"><summary>${localized({ en: "WHY THE OLDER JUDGES WERE REJECTED", ru: "ПОЧЕМУ БОЛЕЕ СТАРЫЕ СУДЬИ ОТКЛОНЕНЫ" })}</summary><p>J1 v0.3 · ${calibrationV3.score}/${calibrationV3.required}</p><p>J1 v0.2 · ${calibrationV2.score}/${calibrationV2.required}</p><p>J1 v0.1 · ${calibration.score}/${calibration.required}</p><p>Phi · ${escapeHTML(phiLoad.status)}</p></details>`;
