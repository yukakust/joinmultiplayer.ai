...
function e007AnswerPilesQwenSandwichShell() {
...
function e007UsedShelfWriterShell() {
...
async function loadE007LocalLibrary() {
  ...
  if (!target) return;
  try {
    const [response, resultResponse, inventoryResponse, readerResponse, cacheResponse, quantResponse] = await Promise.all([
      fetch("/experiments/E007/local-library-discovery-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/local-library-discovery-result-yukabox-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/codex-visible-inventory-yukabox-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/whole-chat-reader-result-v0.2.json", { cache: "no-store" }),
      fetch("/experiments/E007/kv-cache-reuse-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/kv-cache-quantization-result-v0.3.json", { cache: "no-store" }),
    ]);
    ...
    const reader = readerResponse.ok ? await readerResponse.json() : null;
    const cacheData = cacheResponse.ok ? await cacheResponse.json() : null;
    const quantData = quantResponse.ok ? await quantResponse.json() : null;
    ...
    const yukaboxResult = result ? `<div class="e007-result-metrics"><article class="is-correct"><span>YUKABOX · CODEX</span><strong>${result.sources.codex.likely_session_files}</strong><small>${localized({ en: "session files found", ru: "файлов сессий найдено" })}</small></article><article class="needs-review"><span>${localized({ en: "BROAD CANDIDATES", ru: "ШИРОКИХ КАНДИДАТОВ" })}</span><strong>${result.sources.codex.broad_candidate_files}</strong><small>${localized({ en: "most are not conversations", ru: "большинство — не разговоры" })}</small></article><article class="is-correct"><span>${localized({ en: "TEXT EXPOSED", ru: "ТЕКСТА ПОКАЗАНО" })}</span><strong>0</strong></article></div><div class="control-warning"><strong>${localized({ en: "Important finding", ru: "Важная находка" })}</strong><p>${localized({ en: "The Codex folder also contains auth, caches, logs, plugins, attachments, and temporary files. The next adapter must use a strict allowlist. It must never feed the whole folder into search.", ru: "В папке Codex также лежат авторизация, кэши, логи, плагины, вложения и временные файлы. Следующий адаптер обязан использовать строгий белый список. Нельзя целиком отдавать эту папку поиску." })}</p></div>` : "";
    const inventoryResult = inventory ? `<h2>${localized({ en: "How much visible memory is there?", ru: "Сколько видимой памяти найдено?" })}</h2><div class="e007-result-metrics"><article class="is-correct"><span>${localized({ en: "MAIN CONVERSATIONS", ru: "ОСНОВНЫХ РАЗГОВОРОВ" })}</span><strong>${inventory.main_conversations}</strong></article><article class="needs-review"><span>${localized({ en: "CHILD AGENT CONVERSATIONS", ru: "ДОЧЕРНИХ РАЗГОВОРОВ АГЕНТОВ" })}</span><strong>${inventory.child_agent_conversations}</strong></article><article class="is-correct"><span>${localized({ en: "MEDIAN MAIN CHAT", ru: "МЕДИАННЫЙ ОСНОВНОЙ ЧАТ" })}</span><strong>${inventory.main_conversation_tokens.median.toLocaleString()} t</strong></article><article class="needs-review"><span>${localized({ en: "LARGEST MAIN CHAT", ru: "САМЫЙ БОЛЬШОЙ ОСНОВНОЙ ЧАТ" })}</span><strong>${inventory.main_conversation_tokens.max.toLocaleString()} t</strong></article></div><p class="control-warning">${localized({ en: `${inventory.session_files} files became ${inventory.unique_conversations} unique conversations after ${inventory.duplicate_message_records_removed} duplicate message records were removed. Tokens use the local Qwen3 tokenizer.`, ru: `${inventory.session_files} файлов превратились в ${inventory.unique_conversations} уникальных разговоров после удаления ${inventory.duplicate_message_records_removed} повторных записей сообщений. Токены посчитаны локальным токенизатором Qwen3.` })}</p>` : "";
    const readerResult = reader ? `<section id="whole-chat-reader" class="e007-whole-chat-result"><div class="e005-gate4-result-verdict"><span>GATE 16B.1 · ${localized({ en: "DEVELOPMENT TEST PASSED", ru: "DEVELOPMENT-ТЕСТ ПРОШЁЛ" })}</span><h2>${localized({ en: "Qwen found all six facts in two whole chats.", ru: "Qwen нашла все шесть фактов в двух цельных чатах." })}</h2><p>${localized({ en: "One supporting message was cited incorrectly, so the honest score is 11/12. Without the chats the model answered NOT_FOUND six times.", ru: "Одно сообщение-основание указано неверно, поэтому честный балл — 11/12. Без чатов модель шесть раз ответила NOT_FOUND." })}</p></div><div class="e007-result-metrics"><article class="is-correct"><span>${localized({ en: "WITH WHOLE CHATS", ru: "С ЦЕЛЬНЫМИ ЧАТАМИ" })}</span><strong>${reader.summary.with_conversation_points} / ${reader.summary.with_conversation_points_possible}</strong></article><article class="is-correct"><span>${localized({ en: "WITHOUT CHATS", ru: "БЕЗ ЧАТОВ" })}</span><strong>${reader.summary.without_conversation_points} / ${reader.summary.without_conversation_points_possible}</strong></article><article class="is-correct"><span>${localized({ en: "FACTS FOUND", ru: "ФАКТОВ НАЙДЕНО" })}</span><strong>${reader.summary.facts_correct} / ${reader.summary.questions}</strong></article><article class="needs-review"><span>${localized({ en: "CPU READING TIME", ru: "ВРЕМЯ ЧТЕНИЯ НА CPU" })}</span><strong>${Math.round(reader.summary.full_context_seconds / 60)} min</strong></article></div><h2>${localized({ en: "Every question and raw answer", ru: "Каждый вопрос и сырой ответ" })}</h2><div class="e007-whole-chat-cases">${reader.conversations.flatMap(chat => chat.questions.map(item => `<article class="is-score-${item.manual_score}"><header><code>${escapeHTML(item.id)}</code><span>${escapeHTML(chat.label)} · ${chat.tokens.toLocaleString()} t · ${escapeHTML(item.position)}</span><strong>${item.manual_score} / 2</strong></header><h3>${escapeHTML(item.question)}</h3><div><small>${localized({ en: "QWEN ANSWER", ru: "ОТВЕТ QWEN" })}</small><p>${escapeHTML(item.answer)}</p></div><details><summary>${localized({ en: "Why this score?", ru: "Почему такая оценка?" })}</summary><p><b>${localized({ en: "Expected:", ru: "Ожидалось:" })}</b> ${escapeHTML(item.expected)}</p><p><b>${localized({ en: "Evidence:", ru: "Основание:" })}</b> ${item.reported_evidence.map(escapeHTML).join(", ")}</p><p>${escapeHTML(item.manual_reason)}</p></details></article>`)).join("")}</div><div class="control-warning"><strong>${localized({ en: "Small but useful result", ru: "Маленький, но полезный результат" })}</strong><p>${localized({ en: "This is only two child-agent chats and six questions. It supports whole-chat reading around 10k–13k tokens; it does not prove library-wide search or reading chats larger than the context window.", ru: "Это только два дочерних чата и шесть вопросов. Результат поддерживает чтение цельных чатов размером 10–13 тысяч токенов, но не доказывает поиск по всей библиотеке или чтение чатов больше контекстного окна." })}</p></div><div class="actions"><a class="primary-link" href="/experiments/E007/whole-chat-reader-result-v0.2.json">${localized({ en: "OPEN RESULT JSON", ru: "ОТКРЫТЬ RESULT JSON" })} ↗</a><a class="quiet-link" href="/experiments/E007/whole-chat-reader-protocol-v0.2.json">${localized({ en: "LOCKED PROTOCOL", ru: "ЗАМОРОЖЕННЫЙ ПРОТОКОЛ" })} ↗</a></div></section>` : "";
    const cacheResult = cacheData ? `<section id="kv-cache-test" class="e007-whole-chat-result"><div class="e005-gate4-result-verdict is-failed"><span>GATE 16B.2 · ${localized({ en: "STRICT GATE FAILED", ru: "СТРОГИЙ GATE НЕ ПРОШЁЛ" })}</span><h2>${localized({ en: "The cache worked. CPU generation is still slow.", ru: "Кэш сработал. Генерация на CPU всё ещё медленная." })}</h2><p>${localized({ en: "The second question skipped 138 seconds of rereading, but writing 118 tokens still took 84 seconds.", ru: "Второй вопрос пропустил 138 секунд повторного чтения, но написание 118 токенов всё равно заняло 84 секунды." })}</p></div><div class="e007-result-metrics"><article class="is-correct"><span>${localized({ en: "FIRST USE", ru: "ПЕРВЫЙ ВОПРОС" })}</span><strong>${Math.round(cacheData.timing.first_question.first_use_including_prefill_seconds)} s</strong></article><article class="is-correct"><span>${localized({ en: "CACHED SECOND", ru: "ВТОРОЙ ИЗ КЭША" })}</span><strong>${Math.round(cacheData.timing.second_question.cached_total_seconds)} s</strong></article><article class="is-correct"><span>${localized({ en: "SPEEDUP", ru: "УСКОРЕНИЕ" })}</span><strong>${cacheData.timing.second_question_speedup_vs_first_use.toFixed(1)}×</strong></article><article class="needs-review"><span>KV-CACHE</span><strong>${cacheData.cache.gibibytes.toFixed(2)} GiB</strong></article></div><div class="e007-whole-chat-cases">${cacheData.answers.map(item => `<article class="is-score-1"><header><code>${escapeHTML(item.id)}</code><span>${localized({ en: "fact correct · source wrong", ru: "факт верный · ссылка неверная" })}</span><strong>1 / 2</strong></header><h3>${escapeHTML(item.question)}</h3><div><small>${localized({ en: "CACHED QWEN ANSWER", ru: "КЭШИРОВАННЫЙ ОТВЕТ QWEN" })}</small><p>${escapeHTML(item.answer)}</p></div><p><b>${localized({ en: "Expected support:", ru: "Правильная ссылка:" })}</b> ${escapeHTML(item.expected_support)} · <b>${localized({ en: "reported:", ru: "указано:" })}</b> ${escapeHTML(item.reported_support)}</p></article>`).join("")}</div><div class="control-warning"><strong>${localized({ en: "What the 1.48 GiB contains", ru: "Что лежит в этих 1,48 ГиБ" })}</strong><p>${localized({ en: "Not text. Every token keeps key and value vectors in all 36 layers: 36 layers × 8 KV heads × 128 numbers × K and V × 2 BF16 bytes.", ru: "Не текст. Для каждого токена хранятся key- и value-векторы во всех 36 слоях: 36 слоёв × 8 KV-голов × 128 чисел × K и V × 2 байта BF16." })}</p></div><div class="actions"><a class="primary-link" href="/experiments/E007/kv-cache-reuse-result-v0.1.json">RESULT JSON ↗</a><a class="quiet-link" href="/experiments/E007/kv-cache-reuse-protocol-v0.1.json">PROTOCOL ↗</a></div></section>` : "";
    const quantResult = quantData ? `<section id="kv-cache-q4" class="e007-whole-chat-result"><div class="e005-gate4-result-verdict is-failed"><span>GATE 16B.3 · ${localized({ en: "Q4 KV QUALITY GATE FAILED", ru: "Q4 KV НЕ ПРОШЁЛ ПРОВЕРКУ КАЧЕСТВА" })}</span><h2>${localized({ en: "Half the memory. One visible quality failure.", ru: "Почти вдвое меньше памяти. Но качество просело." })}</h2><p>${localized({ en: "Q4 KV saved 576 MiB, but produced a degenerate answer. Q8 remains the safe default.", ru: "Q4 KV сэкономил 576 МиБ, но один ответ превратился в бессмыслицу. Безопасный режим по умолчанию — Q8." })}</p></div><div class="e007-result-metrics"><article class="is-correct"><span>Q8 KV · ${localized({ en: "FACTS", ru: "ФАКТЫ" })}</span><strong>${quantData.summary.q8_kv.fact_score} / 6</strong></article><article class="is-wrong"><span>Q4 KV · ${localized({ en: "FACTS", ru: "ФАКТЫ" })}</span><strong>${quantData.summary.q4_kv.fact_score} / 6</strong></article><article class="is-correct"><span>Q8 KV · ${localized({ en: "MEMORY", ru: "ПАМЯТЬ" })}</span><strong>${quantData.summary.q8_kv.kv_buffer_mib} MiB</strong></article><article class="needs-review"><span>Q4 KV · ${localized({ en: "MEMORY", ru: "ПАМЯТЬ" })}</span><strong>${quantData.summary.q4_kv.kv_buffer_mib} MiB</strong></article><article class="is-correct"><span>Q8 KV · ${localized({ en: "SPEED", ru: "СКОРОСТЬ" })}</span><strong>${quantData.summary.q8_kv.generated_tokens_per_second.toFixed(1)} t/s</strong></article><article class="needs-review"><span>Q4 KV · ${localized({ en: "SPEED", ru: "СКОРОСТЬ" })}</span><strong>${quantData.summary.q4_kv.generated_tokens_per_second.toFixed(1)} t/s</strong></article></div><h2>${localized({ en: "Check every answer yourself", ru: "Проверьте каждый ответ сами" })}</h2><div class="e007-whole-chat-cases">${quantData.cases.map(item => `<article class="is-score-${item.q8.score + item.q4.score}"><header><code>${escapeHTML(item.id)}</code><strong>Q8 ${item.q8.score} · Q4 ${item.q4.score}</strong></header><h3>${escapeHTML(item.question)}</h3><div><small>${localized({ en: "EXPECTED FACT", ru: "ОЖИДАЕМЫЙ ФАКТ" })}</small><p>${escapeHTML(item.expected)}</p></div><div><small>Q8 KV · ${item.q8.score ? "✓" : "×"}</small><p>${escapeHTML(item.q8.answer)}</p><p>${escapeHTML(item.q8.reason)}</p></div><div><small>Q4 KV · ${item.q4.score ? "✓" : "×"}</small><p>${escapeHTML(item.q4.answer)}</p><p>${escapeHTML(item.q4.reason)}</p></div></article>`).join("")}</div><div class="control-warning"><strong>${localized({ en: "Where the speed came from", ru: "Откуда взялось ускорение" })}</strong><p>${localized({ en: "Q4 KV itself added only about 6% generation speed. The big jump—from about 1.4 to 9–10 tokens/s—came from Q4 model weights plus the optimized ROCm runtime.", ru: "Сам Q4 KV добавил лишь около 6% скорости генерации. Большой скачок — примерно с 1,4 до 9–10 токенов/с — дал переход на Q4-веса модели и оптимизированный ROCm-рантайм." })}</p></div><div class="actions"><a class="primary-link" href="/experiments/E007/kv-cache-quantization-result-v0.3.json">RESULT JSON ↗</a><a class="quiet-link" href="/experiments/E007/kv-cache-quantization-protocol-v0.3.json">PROTOCOL ↗</a><a class="quiet-link" href="/experiments/E007/kv-cache-quantization-invalid-attempt-v0.1.json">INVALID ATTEMPT ↗</a></div></section>` : "";
    target.querySelector(".experiment-loading").outerHTML = `<section class="e007-physical-plan"><div class="e005-gate4-result-verdict"><span>${localized({ en: "YUKABOX CHECKED · MACBOOK STILL PENDING", ru: "YUKABOX ПРОВЕРЕН · MACBOOK ЕЩЁ ОЖИДАЕТ" })}</span><h2>${localized(protocol.title)}</h2><p>${localized({ en: "This step sees metadata only. Conversation text stays untouched and invisible.", ru: "Этот шаг видит только метаданные. Текст разговоров остаётся нетронутым и невидимым." })}</p></div>${yukaboxResult}${inventoryResult}${readerResult}${cacheResult}${quantResult}<div class="e007-physical-devices e007-library-sources">${sourceCards}</div><div class="control-warning"><strong>${localized({ en: "Next decision", ru: "Следующее решение" })}</strong><p>${localized({ en: "Whole chats around 10k–13k tokens worked in this small test. Larger chats still need message-level search with exact provenance.", ru: "Цельные чаты на 10–13 тысяч токенов сработали в маленьком тесте. Для больших чатов всё ещё нужен поиск по сообщениям с точными координатами." })}</p></div><h2>${localized({ en: "Run the read-only probe on the MacBook", ru: "Запустить безопасный детектор на MacBook" })}</h2><pre class="experiment-command"><code>${escapeHTML(command)}</code></pre><div class="actions"><a class="primary-link" href="/experiments/E007/codex-visible-inventory-yukabox-v0.1.json">${localized({ en: "VISIBLE MEMORY INVENTORY", ru: "ИНВЕНТАРИЗАЦИЯ ВИДИМОЙ ПАМЯТИ" })} ↗</a><a class="quiet-link" href="/experiments/E007/local-library-discovery-result-yukabox-v0.1.json">${localized({ en: "YUKABOX DISCOVERY", ru: "ОБНАРУЖЕНИЕ НА YUKABOX" })} ↗</a><a class="quiet-link" href="/experiments/E007/local-library-probe-v0.1.py">${localized({ en: "VIEW THE PROBE", ru: "ПОСМОТРЕТЬ КОД ДЕТЕКТОРА" })} ↗</a><a class="quiet-link" href="/experiments/E007/local-library-discovery-protocol-v0.1.json">${localized({ en: "FROZEN RULES", ru: "ЗАФИКСИРОВАННЫЕ ПРАВИЛА" })} ↗</a><a class="quiet-link" href="/experiment/e007/gate-16a/">GATE 16A ↗</a></div></section>`;
  } catch (error) {
...
async function loadE007PhysicalMvp() {
  ...
  try {
    const [protocolResponse, resultResponse, auditResponse] = await Promise.all([
      fetch("/experiments/E007/physical-mvp-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/physical-mvp-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/physical-mvp-human-audit-v0.1.json", { cache: "no-store" }),
    ]);
    if (!protocolResponse.ok) throw new Error("Gate 16A protocol missing");
    ...
    let outcome = `<div class="control-warning"><strong>${localized({ en: "Nothing has run yet", ru: "Запуск ещё не начался" })}</strong><p>${localized({ en: "This page is the promise written before seeing any result: 4 pocket i, 2 physical devices, 6 new questions, 72 search receipts, 10 required sources, 2 preserved alternatives, and zero private-code leaks.", ru: "Это обещание записано до результата: 4 pocket i, 2 физических устройства, 6 новых вопросов, 72 квитанции поиска, 10 обязательных источников, 2 сохранённые альтернативы и ни одной утечки приватного кода." })}</p></div><h2>${localized({ en: "What must cross the wire", ru: "Что должно встретиться через сеть" })}</h2><div class="e007-physical-questions">${questions}</div>`;
    if (resultResponse.ok && auditResponse.ok) {
      const result = await resultResponse.json();
      const audit = await auditResponse.json();
      const auditById = new Map(audit.cases.map(item => [item.id, item]));
      const reasonRU = {
        P01: "Ответ верный. Но безобидная запись о складе ошибочно попала на полку другого мнения.",
        P02: "Ответ верный. Но спорный скопированный отчёт попал в USED вместо отдельной полки версий.",
        P03: "Обе части пришли с разных машин, попали в USED и стали одним верным ответом.",
        P04: "MacBook не отправил безопасный шаг. Центральная часть уже не могла восстановить потерянное знание.",
        P05: "Неопределённость и следующий шаг сохранены. Чужое правило осталось вне ответа, но попало не на ту полку.",
        P06: "Приватный код не утёк. Но сборщик принял публичную инструкцию C4 за приватный код и выдумал ответ.",
      };
      const cards = result.questions.map(item => {
        const review = auditById.get(item.id);
        const received = item.candidates_before_central_filter.map(source => `<li><span class="e007-device-pill">${escapeHTML(source.device)}</span><code>${escapeHTML(source.source_id)}</code><p>${escapeHTML(source.claim)}</p></li>`).join("");
        const used = item.shelves.USED.map(source => `<li><code>${escapeHTML(source.source_id)}</code><p>${escapeHTML(source.claim)}</p></li>`).join("") || `<li>${localized({ en: "Empty", ru: "Пусто" })}</li>`;
        const same = item.shelves.SAME_CASE.map(source => `<li><code>${escapeHTML(source.source_id)}</code><p>${escapeHTML(source.claim)}</p></li>`).join("") || `<li>${localized({ en: "Empty", ru: "Пусто" })}</li>`;
        return `<article class="e007-physical-case ${review.main_answer_correct ? "is-pass" : "is-fail"}"><header><code>${escapeHTML(item.id)}</code><strong>${review.main_answer_correct ? "● " + localized({ en: "CORRECT ANSWER", ru: "ОТВЕТ ВЕРНЫЙ" }) : "× " + localized({ en: "FAILED", ru: "ОШИБКА" })}</strong></header><h2>${escapeHTML(item.question)}</h2><div class="e007-physical-answer"><small>${localized({ en: "FINAL ANSWER", ru: "ФИНАЛЬНЫЙ ОТВЕТ" })}</small><p>${escapeHTML(item.writer.parsed.answer)}</p></div><p class="e007-physical-reason">${escapeHTML(localized({ en: review.reason, ru: reasonRU[item.id] }))}</p><div class="e007-physical-shelves"><section><small>${localized({ en: "USED FOR THE ANSWER", ru: "ИСПОЛЬЗОВАНО В ОТВЕТЕ" })}</small><ul>${used}</ul></section><section><small>${localized({ en: "OTHER VERSION", ru: "ДРУГАЯ ВЕРСИЯ" })}</small><ul>${same}</ul></section></div><details><summary>${localized({ en: "Everything found on both machines", ru: "Всё, что нашли обе машины" })} · ${item.candidates_before_central_filter.length}</summary><ul class="e007-physical-received">${received}</ul></details></article>`;
      }).join("");
      outcome = `<div class="e005-gate4-result-verdict is-failed"><span>${localized({ en: "LOCKED PHYSICAL MVP GATE · FAILED", ru: "ЗАМОРОЖЕННЫЙ ФИЗИЧЕСКИЙ MVP-GATE · НЕ ПРОШЁЛ" })}</span><h2>${localized({ en: "The two machines connected. The shelf still made two critical mistakes.", ru: "Две машины соединились. Полка всё ещё допустила две критические ошибки." })}</h2><p>${localized({ en: "Four ordinary answers are correct. One lost a needed half before the merge. The private question received a made-up answer even though the secret itself stayed on the MacBook.", ru: "Четыре обычных ответа верны. Один потерял нужную половину ещё до сборки. На приватный вопрос система выдумала ответ, хотя сам секрет остался на MacBook." })}</p></div><div class="e007-result-metrics"><article class="is-correct"><span>${localized({ en: "PHYSICAL NODES", ru: "ФИЗИЧЕСКИЕ УЗЛЫ" })}</span><strong>4 / 4</strong></article><article class="is-wrong"><span>${localized({ en: "REQUIRED PIECES", ru: "НУЖНЫЕ ЧАСТИ" })}</span><strong>9 / 10</strong></article><article class="is-wrong"><span>${localized({ en: "ORDINARY ANSWERS", ru: "ОБЫЧНЫЕ ОТВЕТЫ" })}</span><strong>4 / 5</strong></article><article class="is-correct"><span>${localized({ en: "RAW SECRET LEAKS", ru: "УТЕЧКИ СЕКРЕТА" })}</span><strong>0</strong></article><article class="is-wrong"><span>${localized({ en: "PRIVATE TEST", ru: "ПРИВАТНЫЙ ТЕСТ" })}</span><strong>0 / 1</strong></article><article class="is-wrong"><span>${localized({ en: "MADE-UP FACTS", ru: "ВЫДУМАННЫЕ ФАКТЫ" })}</span><strong>1</strong></article></div><h2>${localized({ en: "All six paths—from both machines to the answer", ru: "Все шесть путей — от двух машин до ответа" })}</h2><div class="e007-physical-cases">${cards}</div>`;
    }
    target.querySelector(".experiment-loading").outerHTML = `<section class="e007-physical-plan"><div class="e007-physical-devices"><article><span>MACBOOK</span><h2>MVP-M1 + MVP-M2</h2><p>${localized({ en: "diagnosis A · safe steps B", ru: "диагнозы A · безопасные шаги B" })}</p></article><div class="e007-physical-wire">⇄<small>${localized({ en: "only evidence capsules", ru: "только капсулы с доказательствами" })}</small></div><article><span>YUKABOX</span><h2>MVP-Y1 + MVP-Y2</h2><p>${localized({ en: "safe steps A · diagnosis B", ru: "безопасные шаги A · диагнозы B" })}</p></article></div>${outcome}<div class="actions"><a class="primary-link" href="/experiments/E007/physical-mvp-human-audit-v0.1.json">${localized({ en: "MANUAL AUDIT", ru: "РУЧНАЯ ПРОВЕРКА" })} ↗</a><a class="quiet-link" href="/experiments/E007/physical-mvp-result-v0.1.json">${localized({ en: "FULL TRACE", ru: "ПОЛНЫЙ TRACE" })} ↗</a><a class="quiet-link" href="/experiments/E007/physical-mvp-protocol-v0.1.json">${localized({ en: "LOCKED PLAN", ru: "ЗАМОРОЖЕННЫЙ ПЛАН" })} ↗</a></div></section>`;
  } catch (error) {
...
async function loadE007UsedShelfWriter() {
  ...
  if (!target) return;
  try {
    const [resultResponse, auditResponse] = await Promise.all([
      fetch("/experiments/E007/used-shelf-writer-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/used-shelf-writer-human-audit-v0.1.json", { cache: "no-store" }),
    ]);
    if (!resultResponse.ok || !auditResponse.ok) throw new Error("Gate 15F data missing");
    const result = await resultResponse.json();
    const audit = await auditResponse.json();
    const cards = result.records.map(item => {
      const sources = item.used.map(source => `<article><code>${escapeHTML(source.source_id)}</code><p>${escapeHTML(source.fragment)}</p></article>`).join("");
      return `<article class="e007-writer-case"><header><code>${escapeHTML(item.id)}</code><span>● ${localized({ en: "PASS", ru: "ВЕРНО" })}</span></header><h2>${escapeHTML(item.question)}</h2><div class="e007-writer-flow"><section><small>${localized({ en: "THE TWO PIECES QWEN SAW", ru: "ДВЕ ЧАСТИ, КОТОРЫЕ УВИДЕЛА QWEN" })}</small>${sources}</section><span class="e007-writer-arrow">→</span><section><small>${localized({ en: "ONE HUMAN ANSWER", ru: "ОДИН ЧЕЛОВЕЧЕСКИЙ ОТВЕТ" })}</small><p class="e007-ledger-answer">${escapeHTML(item.parsed.answer)}</p><p><code>${escapeHTML(item.parsed.evidence_ids.join(" + "))}</code></p></section></div></article>`;
    }).join("");
    target.querySelector(".experiment-loading").outerHTML = `<section class="e007-cluster-result"><div class="e005-gate4-result-verdict"><span>${localized({ en: "LOCKED SYNTHETIC DEVELOPMENT GATE · PASSED", ru: "ЗАМОРОЖЕННЫЙ SYNTHETIC DEVELOPMENT-GATE · ПРОШЁЛ" })}</span><h2>${localized({ en: "The correct shelf became a complete answer 30 out of 30 times.", ru: "Правильная полка стала полным ответом в 30 случаях из 30." })}</h2><p>${localized({ en: "Every answer kept the supported cause or uncertainty and the safe action or next measurement. No new fact or hidden alternative entered the answer.", ru: "Каждый ответ сохранил подтверждённую причину или неопределённость и безопасное действие или следующее измерение. Ни новый факт, ни скрытая версия в ответ не попали." })}</p></div><div class="e007-result-metrics"><article class="is-correct"><span>${localized({ en: "COMPLETE ANSWERS", ru: "ПОЛНЫЕ ОТВЕТЫ" })}</span><strong>30 / 30</strong></article><article class="is-correct"><span>${localized({ en: "MADE-UP FACTS", ru: "ВЫДУМАННЫЕ ФАКТЫ" })}</span><strong>0</strong></article><article class="is-correct"><span>${localized({ en: "ALTERNATIVE LEAKS", ru: "УТЕЧКИ ДРУГИХ ВЕРСИЙ" })}</span><strong>0</strong></article></div><div class="control-warning"><strong>${localized({ en: "Honest boundary", ru: "Честная граница" })}</strong><p>${localized({ en: "This closes the writer-from-correct-shelf step on known synthetic English cases. It does not show that the full chain finds the correct shelf on new data or across physical devices.", ru: "Мы закрыли только шаг «правильная полка → ответ» на знакомых синтетических английских примерах. Это ещё не доказывает, что вся цепочка найдёт правильную полку на новых данных и физических устройствах." })}</p></div><h2>${localized({ en: "All 30 questions, inputs, and answers", ru: "Все 30 вопросов, входов и ответов" })}</h2><div class="e007-writer-cases">${cards}</div><div class="actions"><a class="primary-link" href="/experiments/E007/used-shelf-writer-human-audit-v0.1.json">${localized({ en: "MANUAL AUDIT", ru: "РУЧНАЯ ПРОВЕРКА" })} ↗</a><a class="quiet-link" href="/experiments/E007/used-shelf-writer-result-v0.1.json">${localized({ en: "FULL RAW TRACE", ru: "ПОЛНЫЙ СЫРОЙ TRACE" })} ↗</a><a class="quiet-link" href="/experiments/E007/used-shelf-writer-protocol-v0.1.json">${localized({ en: "PLAN BEFORE RUN", ru: "ПЛАН ДО ЗАПУСКА" })} ↗</a></div></section>`;
  } catch (error) {
    target.querySelector(".experiment-loading").textContent = localized({ en: "Gate 15F could not be loaded.", ru: "Не удалось загрузить Gate 15F." });
  }
}
...
async function loadE007Qwen8BBundle() {
  ...
  try {
    const [protocolResponse, resultResponse, auditResponse] = await Promise.all([
      fetch("/experiments/E007/qwen8b-bundle-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/qwen8b-bundle-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/qwen8b-bundle-human-audit-v0.1.json", { cache: "no-store" }),
    ]);
    if (!protocolResponse.ok || !resultResponse.ok || !auditResponse.ok) throw new Error("Gate 15D data missing");
    const protocol = await protocolResponse.json();
    const result = await resultResponse.json();
    const audit = await auditResponse.json();
    const failed = new Map(audit.failed_cases.map(item => [item.id, item.reason]));
    const cases = result.records.map(item => {
      const isPassed = !failed.has(item.id);
      const answer = item.answer.parsed || {};
      const selected = new Set(item.selector.selected_ids);
      const selectedSources = item.offers.filter(source => selected.has(source.source_id)).map(source => `<li><code>${escapeHTML(source.source_id)}</code><span>${escapeHTML(source.fragment)}</span></li>`).join("");
      const alternativeSources = item.offers.filter(source => item.gold.alternatives.includes(source.source_id)).map(source => `<li><code>${escapeHTML(source.source_id)}</code><span>${escapeHTML(source.fragment)}</span></li>`).join("");
      const expected = [item.expected.cause, item.expected.action].filter(Boolean).join(" ");
      const actual = [answer.best_supported, answer.action_or_next_step].filter(Boolean).join(" ");
      return `<article class="e007-bundle-case ${isPassed ? "is-passed" : "is-failed"}"><header><span>${isPassed ? "● " + localized({ en: "PASS", ru: "ВЕРНО" }) : "× " + localized({ en: "FAIL", ru: "ОШИБКА" })}</span><code>${escapeHTML(item.id)}</code></header><h3>${escapeHTML(item.question)}</h3><div class="e007-bundle-answer-pair"><section><small>${localized({ en: "EXPECTED", ru: "ОЖИДАЛОСЬ" })}</small><p>${escapeHTML(expected)}</p></section><section><small>QWEN3-8B</small><p>${escapeHTML(actual)}</p></section></div>${isPassed ? "" : `<p class="e007-bundle-reason">${escapeHTML(failed.get(item.id))}</p>`}<details><summary>${localized({ en: "Selected source fragments", ru: "Выбранные фрагменты" })} · ${item.selector.selected_ids.length}</summary><ul>${selectedSources}</ul></details>${alternativeSources ? `<details class="e007-bundle-dropped"><summary>${localized({ en: "Same-case alternatives that were dropped", ru: "Отброшенные альтернативы того же случая" })} · ${item.gold.alternatives.length}</summary><ul>${alternativeSources}</ul></details>` : ""}<details><summary>${localized({ en: "Full structured answer", ru: "Полный структурированный ответ" })}</summary><pre>${escapeHTML(JSON.stringify(answer, null, 2))}</pre></details></article>`;
    }).join("");
    target.querySelector(".experiment-loading").outerHTML = `<section class="e007-cluster-result"><div class="e005-gate4-result-verdict is-failed"><span>${localized({ en: "LOCKED SYNTHETIC DEVELOPMENT RESULT · FAILED", ru: "ЗАМОРОЖЕННЫЙ SYNTHETIC DEVELOPMENT-РЕЗУЛЬТАТ · НЕ ПРОШЁЛ" })}</span><h2>${localized({ en: "The puzzle repaired X → Y. It erased the opposing views.", ru: "Пазл починил X → Y. Но стёр спорные версии." })}</h2><p>${localized({ en: "Qwen selected every required cause and action, rejected every frozen irrelevant fragment, and wrote the correct core answer in all 30 cases. But it dropped all 24 same-case alternatives, so only 16 complete answers passed the strict rubric.", ru: "Qwen выбрала все нужные причины и действия, отбросила все замороженные чужие фрагменты и дала правильный основной ответ во всех 30 случаях. Но она выбросила все 24 альтернативы того же случая — поэтому строгую проверку прошли только 16 полных ответов." })}</p></div><div class="e007-result-metrics"><article class="is-correct"><span>${localized({ en: "CAUSE + ACTION", ru: "ПРИЧИНА + ДЕЙСТВИЕ" })}</span><strong>30 / 30</strong></article><article class="is-correct"><span>${localized({ en: "IRRELEVANT PASSED", ru: "ЧУЖОГО ПРОПУЩЕНО" })}</span><strong>0 / 396</strong></article><article class="is-wrong"><span>${localized({ en: "FULL ANSWERS", ru: "ПОЛНЫХ ОТВЕТОВ" })}</span><strong>${audit.summary.fully_correct} / ${audit.summary.questions}</strong><small>${localized({ en: "needed 24", ru: "нужно 24" })}</small></article></div><div class="e007-result-metrics"><article class="is-correct"><span>${localized({ en: "CONDITIONAL RULES KEPT", ru: "УСЛОВНЫХ ПРАВИЛ СОХРАНЕНО" })}</span><strong>30 / 30</strong></article><article class="is-wrong"><span>${localized({ en: "ALTERNATIVES KEPT", ru: "АЛЬТЕРНАТИВ СОХРАНЕНО" })}</span><strong>0 / 24</strong></article><article class="is-wrong"><span>${localized({ en: "UNSUPPORTED ADDITIONS", ru: "НЕПОДТВЕРЖДЁННЫХ ДОБАВОК" })}</span><strong>${audit.summary.unsupported_new_facts}</strong></article></div><div class="control-warning"><strong>${localized({ en: "The architectural lesson", ru: "Архитектурный вывод" })}</strong><p>${localized({ en: "One bundle view is excellent for finding the best supported answer and joining conditional rules. It is not enough for preserving the evidence landscape: a separate shelf must mechanically retain same-case alternatives and their lineages before final synthesis.", ru: "Один общий взгляд на папку отлично находит лучший ответ и соединяет условные правила. Но этого мало для сохранения всей картины доказательств: отдельная полка должна механически удерживать альтернативы того же случая и их происхождение до финальной сборки." })}</p></div><h2>${localized({ en: "All 30 questions and answers", ru: "Все 30 вопросов и ответов" })}</h2><div class="e007-bundle-cases">${cases}</div><div class="actions"><a class="primary-link" href="/experiments/E007/qwen8b-bundle-human-audit-v0.1.json">${localized({ en: "MANUAL AUDIT", ru: "РУЧНАЯ ПРОВЕРКА" })} ↗</a><a class="quiet-link" href="/experiments/E007/qwen8b-bundle-result-v0.1.json">${localized({ en: "RAW FULL TRACE", ru: "ПОЛНЫЙ СЫРОЙ TRACE" })} ↗</a><a class="quiet-link" href="/experiments/E007/qwen8b-bundle-protocol-v0.1.json">${localized({ en: "PLAN BEFORE RUN", ru: "ПЛАН ДО ЗАПУСКА" })} ↗</a><a class="quiet-link" href="/experiment/e007/gate-15c/">${localized({ en: "PAIRWISE FAILURE", ru: "ПРОВАЛ ПО ОДНОМУ ФРАГМЕНТУ" })} ↗</a></div></section>`;
  } catch (error) {
...
async function loadE007Qwen8BRelevance() {
  ...
  try {
    const [protocolResponse, resultResponse, auditResponse] = await Promise.all([
      fetch("/experiments/E007/qwen8b-relevance-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/qwen8b-relevance-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/qwen8b-relevance-human-audit-v0.1.json", { cache: "no-store" }),
    ]);
    if (!protocolResponse.ok || !resultResponse.ok || !auditResponse.ok) throw new Error("Gate 15C data missing");
    const protocol = await protocolResponse.json();
    const result = await resultResponse.json();
    const audit = await auditResponse.json();
    const lost = new Set(audit.lost_required_ids);
    const errorCards = result.records.filter(item => !item.correct).map(item => {
      const kind = lost.has(item.id) ? "lost" : "ambiguous";
      const label = kind === "lost"
        ? localized({ en: "LOST USEFUL NOTE", ru: "ПОТЕРЯНА НУЖНАЯ ЗАПИСЬ" })
        : localized({ en: "REDUNDANT, BUT SAME CASE", ru: "ЛИШНЕЕ, НО О ТОМ ЖЕ СЛУЧАЕ" });
      const expected = item.gold === "USEFUL"
        ? localized({ en: "should move forward", ru: "нужно было пропустить" })
        : localized({ en: "frozen gold says stop", ru: "замороженная разметка: остановить" });
      const actual = item.decision === "USEFUL"
        ? localized({ en: "Qwen moved it forward", ru: "Qwen пропустила" })
        : localized({ en: "Qwen stopped it", ru: "Qwen остановила" });
      return `<article class="e007-relevance-error ${kind}"><header><span>${label}</span><code>${escapeHTML(item.id)}</code></header><h3>${escapeHTML(item.question)}</h3><blockquote>${escapeHTML(item.fragment)}</blockquote><p><strong>${escapeHTML(actual)}</strong> · ${escapeHTML(expected)}</p></article>`;
    }).join("");
    target.querySelector(".experiment-loading").outerHTML = `<section class="e007-cluster-result"><div class="e005-gate4-result-verdict is-failed"><span>${localized({ en: "LOCKED DEVELOPMENT GATE · FAILED", ru: "ЗАМОРОЖЕННЫЙ DEVELOPMENT-GATE · НЕ ПРОШЁЛ" })}</span><h2>${localized({ en: "8B stopped all clear junk—and threw away nineteen useful pieces.", ru: "8B остановила весь явный мусор — и выбросила девятнадцать полезных частей." })}</h2><p>${localized({ en: "The larger model became stricter, not better for this job. It kept diagnosis notes but rejected action rules that need another fragment to activate them.", ru: "Большая модель стала строже, а не лучше для этой задачи. Она сохранила записи с диагнозом, но отвергла правила действий, которым для срабатывания нужен другой фрагмент." })}</p></div><div class="e007-result-metrics"><article class="is-wrong"><span>${localized({ en: "REQUIRED KEPT", ru: "НУЖНЫХ СОХРАНЕНО" })}</span><strong>${result.summary.required_kept} / ${result.summary.required_total}</strong><small>${localized({ en: "needed 57", ru: "нужно 57" })}</small></article><article class="is-correct"><span>${localized({ en: "EXTRA PASSED", ru: "ЛИШНИХ ПРОПУЩЕНО" })}</span><strong>${result.summary.unrelated_kept} / ${result.summary.unrelated_total}</strong><small>${localized({ en: "needed at most 42", ru: "нужно не больше 42" })}</small></article><article class="is-correct"><span>${localized({ en: "CLEAR JUNK PASSED", ru: "ЯВНОГО МУСОРА ПРОПУЩЕНО" })}</span><strong>0</strong><small>${localized({ en: "manual audit", ru: "ручная проверка" })}</small></article></div><div class="e007-relevance-compare e007-relevance-compare-three"><article><small>4B RERANKER</small><strong>60 ${localized({ en: "required", ru: "нужных" })} · 345 ${localized({ en: "extra", ru: "лишних" })}</strong></article><span>→</span><article><small>QWEN3-1.7B</small><strong>53 ${localized({ en: "required", ru: "нужных" })} · 47 ${localized({ en: "extra", ru: "лишних" })}</strong></article><span>→</span><article><small>QWEN3-8B</small><strong>41 ${localized({ en: "required", ru: "нужных" })} · 18 ${localized({ en: "extra", ru: "лишних" })}</strong></article></div><div class="control-warning"><strong>${localized({ en: "What the 37 mistakes mean", ru: "Что означают 37 ошибок" })}</strong><p>${localized({ en: "All 19 lost fragments are useful: 15 safety rules and 4 instructions for the next measurement. All 18 passed extras are copied reports about the exact same case—not a foreign device. The binary word ‘directly’ rewards strictness and punishes multi-fragment composition.", ru: "Все 19 потерянных фрагментов полезны: 15 правил безопасности и 4 указания, какое измерение сделать дальше. Все 18 пропущенных лишних — копии сообщений о том же самом случае, а не о чужом устройстве. Слово «напрямую» поощряет строгость и ломает сборку ответа из нескольких частей." })}</p></div><h2>${localized({ en: "Every disputed decision", ru: "Все спорные решения" })}</h2><div class="e007-relevance-errors">${errorCards}</div><div class="actions"><a class="primary-link" href="/experiments/E007/qwen8b-relevance-human-audit-v0.1.json">${localized({ en: "MANUAL AUDIT", ru: "РУЧНАЯ ПРОВЕРКА" })} ↗</a><a class="quiet-link" href="/experiments/E007/qwen8b-relevance-result-v0.1.json">${localized({ en: "ALL 480 RAW DECISIONS", ru: "ВСЕ 480 СЫРЫХ РЕШЕНИЙ" })} ↗</a><a class="quiet-link" href="/experiments/E007/qwen8b-relevance-protocol-v0.1.json">${localized({ en: "PLAN BEFORE RUN", ru: "ПЛАН ДО ЗАПУСКА" })} ↗</a><a class="quiet-link" href="/experiment/e007/gate-15b/">1.7B ${localized({ en: "RESULT", ru: "РЕЗУЛЬТАТ" })} ↗</a></div></section>`;
  } catch (error) {
...
async function loadE007Qwen17BRelevance() {
  ...
  try {
    const [protocolResponse, resultResponse, auditResponse] = await Promise.all([
      fetch("/experiments/E007/qwen17b-relevance-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/qwen17b-relevance-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/qwen17b-relevance-human-audit-v0.1.json", { cache: "no-store" }),
    ]);
    if (!protocolResponse.ok || !resultResponse.ok || !auditResponse.ok) throw new Error("Gate 15B data missing");
    const protocol = await protocolResponse.json();
    const result = await resultResponse.json();
    const audit = await auditResponse.json();
    const lost = new Set(audit.lost_required_ids);
    const ambiguous = new Set(audit.ambiguous_redundant_pass_ids);
    const errorCards = result.records.filter(item => !item.correct).map(item => {
      const kind = lost.has(item.id) ? "lost" : (ambiguous.has(item.id) ? "ambiguous" : "wrong");
      const label = kind === "lost"
        ? localized({ en: "LOST USEFUL NOTE", ru: "ПОТЕРЯНА НУЖНАЯ ЗАПИСЬ" })
        : kind === "ambiguous"
          ? localized({ en: "REDUNDANT, BUT RELATED", ru: "ЛИШНЕЕ, НО ПО ТЕМЕ" })
          : localized({ en: "WRONG DEVICE OR SITUATION", ru: "ЧУЖОЕ УСТРОЙСТВО ИЛИ СЛУЧАЙ" });
      const expected = item.gold === "USEFUL"
        ? localized({ en: "should move forward", ru: "нужно было пропустить" })
        : localized({ en: "should stop", ru: "нужно было остановить" });
      const actual = item.decision === "USEFUL"
        ? localized({ en: "Qwen moved it forward", ru: "Qwen пропустила" })
        : localized({ en: "Qwen stopped it", ru: "Qwen остановила" });
      return `<article class="e007-relevance-error ${kind}"><header><span>${label}</span><code>${escapeHTML(item.id)}</code></header><h3>${escapeHTML(item.question)}</h3><blockquote>${escapeHTML(item.fragment)}</blockquote><p><strong>${escapeHTML(actual)}</strong> · ${escapeHTML(expected)}</p></article>`;
    }).join("");
    target.querySelector(".experiment-loading").outerHTML = `<section class="e007-cluster-result"><div class="e005-gate4-result-verdict is-failed"><span>${localized({ en: "LOCKED DEVELOPMENT GATE · FAILED", ru: "ЗАМОРОЖЕННЫЙ DEVELOPMENT-GATE · НЕ ПРОШЁЛ" })}</span><h2>${localized({ en: "The flood shrank. Seven useful safety notes sank with it.", ru: "Поток лишнего почти исчез. Но вместе с ним утонули семь нужных правил безопасности." })}</h2><p>${localized({ en: "Qwen3-1.7B cut unrelated passes from 345 to 47, but kept only 53 of 60 required notes. We did not tune or rerun after seeing the result.", ru: "Qwen3‑1.7B сократила лишние пропуски с 345 до 47, но сохранила лишь 53 из 60 нужных записей. После результата мы ничего не подкручивали и не перезапускали." })}</p></div><div class="e007-result-metrics"><article class="is-wrong"><span>${localized({ en: "REQUIRED KEPT", ru: "НУЖНЫХ СОХРАНЕНО" })}</span><strong>${result.summary.required_kept} / ${result.summary.required_total}</strong><small>${localized({ en: "needed 57", ru: "нужно 57" })}</small></article><article class="is-wrong"><span>${localized({ en: "UNRELATED PASSED", ru: "ЛИШНИХ ПРОПУЩЕНО" })}</span><strong>${result.summary.unrelated_kept} / ${result.summary.unrelated_total}</strong><small>${localized({ en: "needed at most 42", ru: "нужно не больше 42" })}</small></article><article class="is-correct"><span>${localized({ en: "MODEL ANSWERS READABLE", ru: "ОТВЕТЫ МОДЕЛИ ПОНЯТНЫ" })}</span><strong>${result.summary.unparseable}</strong><small>${localized({ en: "unparseable", ru: "неразобранных" })}</small></article></div><div class="e007-relevance-compare"><article><small>${localized({ en: "OLD 4B RERANKER", ru: "СТАРЫЙ 4B RERANKER" })}</small><strong>60 ${localized({ en: "useful", ru: "нужных" })} · 345 ${localized({ en: "extra", ru: "лишних" })}</strong></article><span>→</span><article><small>QWEN3-1.7B</small><strong>53 ${localized({ en: "useful", ru: "нужных" })} · 47 ${localized({ en: "extra", ru: "лишних" })}</strong></article></div><div class="control-warning"><strong>${localized({ en: "What manual reading revealed", ru: "Что показала ручная проверка" })}</strong><p>${localized({ en: "Seven lost notes were safety rules that become useful only together with a separate cause. Of the 47 extra passes, 29 are clearly unrelated. Eighteen are copied reports about the same case: redundant, but reasonably related. The frozen score still counts all 47 as extra.", ru: "Семь потерянных записей — правила безопасности, которые становятся полезными только вместе с отдельной причиной. Из 47 лишних пропусков 29 явно чужие. Ещё 18 — повторные сообщения о том же случае: они лишние, но всё же по теме. В замороженном результате все 47 по-прежнему считаются лишними." })}</p></div><h2>${localized({ en: "Every mistake — read it yourself", ru: "Все ошибки — можно проверить глазами" })}</h2><div class="e007-relevance-errors">${errorCards}</div><div class="actions"><a class="primary-link" href="/experiments/E007/qwen17b-relevance-human-audit-v0.1.json">${localized({ en: "MANUAL AUDIT", ru: "РУЧНАЯ ПРОВЕРКА" })} ↗</a><a class="quiet-link" href="/experiments/E007/qwen17b-relevance-result-v0.1.json">${localized({ en: "ALL 480 RAW DECISIONS", ru: "ВСЕ 480 СЫРЫХ РЕШЕНИЙ" })} ↗</a><a class="quiet-link" href="/experiments/E007/qwen17b-relevance-protocol-v0.1.json">${localized({ en: "PLAN BEFORE RUN", ru: "ПЛАН ДО ЗАПУСКА" })} ↗</a><a class="quiet-link" href="/experiment/e007/gate-15a/">${localized({ en: "FULL PIPELINE X-RAY", ru: "РЕНТГЕН ВСЕГО PIPELINE" })} ↗</a></div></section>`;
  } catch (error) {
...
async function loadE007FullPipeline() {
  ...
  try {
    ...
    const verdictName = value => ({ pass: localized({ en: "CORRECT", ru: "ВЕРНО" }), partial: localized({ en: "PARTLY", ru: "ЧАСТИЧНО" }), fail: localized({ en: "WRONG", ru: "НЕВЕРНО" }) }[value]);
    const expectedText = expected => [expected.cause, expected.action].filter(Boolean).join(" ");
    const cases = result.records.map(record => {
      const review = auditById.get(record.id);
      const accepted = record.capsules.filter(item => item.accepted);
      const evidence = accepted.map(item => `<li><b>${escapeHTML(item.source_id)}</b><span>${escapeHTML(item.claim)}</span></li>`).join("") || `<li>${localized({ en: "Nothing reached the evidence board.", ru: "До доски доказательств ничего не дошло." })}</li>`;
      return `<details class="e007-full-case is-${review.verdict}" ${review.verdict === "fail" ? "open" : ""}><summary><b>${escapeHTML(record.id)}</b><span>${escapeHTML(record.question)}</span><strong>${escapeHTML(verdictName(review.verdict))}</strong></summary><div class="e007-full-answer-grid"><article><small>${localized({ en: "EXPECTED", ru: "ОЖИДАЛИ" })}</small><p>${escapeHTML(expectedText(record.expected))}</p></article><article><small>QWEN3-1.7B</small><p>${escapeHTML(record.final.answer)}</p></article></div><p class="e007-full-review">${escapeHTML(review.reason)}</p><details class="e007-full-evidence"><summary>${localized({ en: "SHOW ACCEPTED CLAIMS", ru: "ПОКАЗАТЬ ПРИНЯТЫЕ УТВЕРЖДЕНИЯ" })} · ${accepted.length}</summary><ul>${evidence}</ul></details></details>`;
    }).join("");
...
async function loadE007() {
  ...
  try {
    ...
    const attentionMarkup = attention ? `<section id="e007-attention" class="e007-smoke-lock"><div class="flow-step">CHECKPOINT 3A · ${localized({ en: "LOCKED BEFORE THE FIRST QUESTION", ru: "ЗАФИКСИРОВАНО ДО ПЕРВОГО ВОПРОСА" })}</div><h2>${escapeHTML(localized(attention.title))}</h2><p>${localized({ en: "We test only two things: the question arrives unchanged, and four public cards return visible attention signals.", ru: "Проверяем только две вещи: вопрос доходит без изменений, а четыре публичные карточки возвращают видимые сигналы внимания." })}</p><div class="e007-device-grid">${attention.cards.map((card) => `<article><span>${escapeHTML(card.id)} · ${escapeHTML(card.device)}</span><h2>${escapeHTML(localized(card.name))}</h2><p>${escapeHTML(card.description.split(";")[language === "ru" ? 1 : 0].trim())}</p></article>`).join("")}</div><p class="control-warning">${localized({ en: "No Qwen, memory, RAG, training, or answer generation. Scores rank where to look first; they are not probabilities or knowledge.", ru: "Без Qwen, памяти, RAG, обучения и генерации ответа. Баллы лишь подсказывают, где искать сначала; это не вероятность и не знание." })}</p><div class="actions"><a class="quiet-link" href="/experiments/E007/attention-protocol-v0.1.json">LOCKED PROTOCOL JSON ↗</a></div></section>` : "";
    const attentionResultsMarkup = attentionRuns?.runs?.length ? attentionRuns.runs.map((run) => {
      const ranked = [...run.nodes].sort((left, right) => (right.response?.whole_text_vector || 0) - (left.response?.whole_text_vector || 0));
    ...
    if (localOfferResult) {
      const stateNames = {
        found: localized({ en: "Found something useful", ru: "Нашла полезное" }),
        empty: localized({ en: "Nothing useful", ru: "Ничего полезного" }),
        blocked: localized({ en: "Private—did not share", ru: "Приватное — не раскрыла" }),
      };
      const tableGroups = localOfferResult.questions_review.map((question) => {
        const cards = Object.entries(question.methods.multilingual_neural);
        return `<tbody>${cards.map(([card, item], index) => {
          const critical = !item.correct && ((item.expected === "found" && item.actual !== "found") || (item.expected === "blocked" && item.actual !== "blocked"));
          const rowClass = item.correct ? "is-correct" : critical ? "is-critical" : "needs-review";
          const verdict = item.correct
            ? `✓ ${localized({ en: "RIGHT", ru: "ВЕРНО" })}`
            : critical
              ? `× ${localized({ en: "KNOWLEDGE LOST", ru: "ЗНАНИЕ ПОТЕРЯНО" })}`
              : `△ ${localized({ en: "FILTER DOWNSTREAM", ru: "ОТФИЛЬТРОВАТЬ НА ПРИЁМКЕ" })}`;
          return `<tr class="${rowClass}">${index === 0 ? `<th rowspan="${cards.length}"><span>${escapeHTML(question.id)}</span>${escapeHTML(question.question)}</th>` : ""}<td><b>${escapeHTML(card)}</b></td><td><span class="e007-state">${escapeHTML(stateNames[item.expected])}</span></td><td><span class="e007-state">${escapeHTML(stateNames[item.actual])}</span></td><td>${item.actual === "empty" ? "—" : escapeHTML(item.source_title)}</td><td class="e007-verdict">${verdict}</td></tr>`;
        }).join("")}</tbody>`;
      }).join("");
      const neural = localOfferResult.lane_summaries.multilingual_neural;
    ...
    let spanBridgeMarkup = "";
    if (spanBridgeResult) {
      const rows = spanBridgeResult.records.map((record) => {
        const tone = record.bridge_correct ? "is-correct" : "is-critical";
        const expected = record.expected === "useful" ? localized({ en: "useful", ru: "полезный" }) : localized({ en: "extra", ru: "лишний" });
        const finalDecision = record.bridge_accepts ? localized({ en: "ACCEPTED", ru: "ПРИНЯТ" }) : localized({ en: "REJECTED", ru: "ОТКЛОНЁН" });
        return `<tr class="${tone}"><th><span>${escapeHTML(record.id)} · ${escapeHTML(record.question_id)}</span>${escapeHTML(record.question)}</th><td><b>${escapeHTML(record.selected_span_id || "NONE")}</b><small>${escapeHTML(record.selected_span || localized({ en: "No span selected", ru: "Фрагмент не выбран" }))}</small></td><td>${escapeHTML(expected)}</td><td><pre class="e007-raw-output">${escapeHTML(record.bridge_raw || localized({ en: "Bridge did not run", ru: "Второй проход не запускался" }))}</pre></td><td class="e007-verdict">${escapeHTML(finalDecision)}</td></tr>`;
      }).join("");
    ...
    if (mobileRerankerResult?.methods?.length) {
      ...
      const preflightRam = mobileRerankerResult.q4_phone_shaped_yukabox_preflight?.max_resident_set_bytes;
    ...
    if (semanticResult?.records?.length) {
      ...
      if (semanticAudit) {
        semanticMarkup += `<section class="e007-local-result-wide e007-interface-audit"><div class="flow-step">POSTHOC · ${localized({ en: "PROMPT AUDIT", ru: "ПРОВЕРКА PROMPT" })}</div><h2>${localized({ en: "Qwen can print NO. It did not use NO as a reliable meaning decision.", ru: "Qwen умеет напечатать NO. Но не умеет надёжно выбирать его по смыслу." })}</h2><div class="e007-result-metrics"><article class="is-correct"><span>${localized({ en: "COPY [NO]", ru: "СКОПИРОВАТЬ [NO]" })}</span><strong>[NO]</strong><small>${localized({ en: "can emit the token", ru: "токен напечатать может" })}</small></article><article class="is-critical"><span>2 + 2 = 5?</span><strong>YES</strong><small>${localized({ en: "semantic answer wrong", ru: "ответ по смыслу неверен" })}</small></article><article class="is-critical"><span>${localized({ en: "OPPOSITE QUOTE", ru: "ПРОТИВОПОЛОЖНАЯ ЦИТАТА" })}</span><strong>YES</strong><small>${localized({ en: "conflict missed", ru: "противоречие потеряно" })}</small></article></div><div class="e005-gate4-result-verdict"><span>${localized({ en: "WHAT CHANGED WHEN LABELS MOVED", ru: "ЧТО СЛУЧИЛОСЬ ПРИ ПЕРЕСТАНОВКЕ МЕТОК" })}</span><h2>${localized({ en: "The model kept choosing the same letter even when that letter changed from NO to YES.", ru: "Модель продолжала выбирать ту же букву, даже когда её смысл поменялся с НЕТ на ДА." })}</h2><p>${escapeHTML(localized(semanticAudit.conclusion))}</p></div><div class="actions"><a class="quiet-link" href="/experiments/E007/two-link-semantic-interface-audit-v0.1.json">${localized({ en: "ALL DIAGNOSTICS", ru: "ВСЕ ДИАГНОСТИЧЕСКИЕ ПРОВЕРКИ" })} ↗</a></div></section>`;
      }
    ...
    if (sourceAnchorProtocol) {
      const scenarioRu = {
        SA01:"точная английская выдержка", SA02:"точная русская выдержка в UTF-8",
        SA03:"повторяющаяся фраза с правильными координатами", SA04:"точная выдержка с переносами CRLF",
        SA05:"начало сдвинуто на один байт", SA06:"конец укорочен на один байт",
        SA07:"выдержку пересказали своими словами", SA08:"hash фрагмента изменён",
        SA09:"источник изменился после создания капсулы", SA10:"источник с таким id отсутствует",
        SA11:"версия источника отсутствует", SA12:"диапазон выходит за конец источника",
        SA13:"отрицательная начальная координата", SA14:"вместо байтов прислали номера символов",
        SA15:"фраза встречается дважды, а координат нет", SA16:"в выдержке подменён похожий Unicode-символ",
        SA17:"переносы LF изменились на CRLF", SA18:"пустой диапазон байтов",
        SA19:"hash верный, но показываемый текст другой", SA20:"источник существует, но его нельзя прочитать",
      };
      const scenario = item => localized({ en:item.scenario, ru:scenarioRu[item.id] || item.scenario });
      const label = value => ({
...
async function loadE007TenButtons() {
  ...
  if (!target) return;
  try {
    const [protocolResponse, resultResponse, sanityProtocolResponse, sanityResultResponse, ladderProtocolResponse, ladderResultResponse, ninetyProtocolResponse, ninetyResultResponse, reversedProtocolResponse, reversedResultResponse, numericProtocolResponse, numericResultResponse, calibrationProtocolResponse, calibrationResultResponse, nonceProtocolResponse, nonceResultResponse, phraseProtocolResponse, phraseResultResponse, nliProtocolResponse, nliResultResponse, debertaProtocolResponse, debertaResultResponse, freshProtocolResponse, freshResultResponse, contextProtocolResponse, contextResultResponse] = await Promise.all([
      ...
      fetch("/experiments/E007/context-ladder-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/context-ladder-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/ninety-word-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/ninety-word-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/ninety-word-reversed-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/ninety-word-reversed-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/numeric-letter-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/numeric-letter-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/mirrored-calibration-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/mirrored-calibration-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/nonce-word-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/nonce-word-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/phrase-length-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/phrase-length-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/nli-minilm-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/nli-minilm-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/nli-deberta-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/nli-deberta-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/nli-fresh20-short-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/nli-fresh20-short-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/nli-fresh20-context-protocol-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/nli-fresh20-context-result-v0.1.json", { cache: "no-store" }),
    ]);
    if (!protocolResponse.ok || !resultResponse.ok || !sanityProtocolResponse.ok || !sanityResultResponse.ok || !ladderProtocolResponse.ok || !ladderResultResponse.ok || !ninetyProtocolResponse.ok || !ninetyResultResponse.ok || !reversedProtocolResponse.ok || !reversedResultResponse.ok || !numericProtocolResponse.ok || !numericResultResponse.ok || !calibrationProtocolResponse.ok || !calibrationResultResponse.ok || !nonceProtocolResponse.ok || !nonceResultResponse.ok || !phraseProtocolResponse.ok || !phraseResultResponse.ok || !nliProtocolResponse.ok || !nliResultResponse.ok || !debertaProtocolResponse.ok || !debertaResultResponse.ok || !freshProtocolResponse.ok || !freshResultResponse.ok || !contextProtocolResponse.ok || !contextResultResponse.ok) throw new Error("compact result unavailable");
    const protocol = await protocolResponse.json();
    const result = await resultResponse.json();
    const sanityProtocol = await sanityProtocolResponse.json();
    const sanityResult = await sanityResultResponse.json();
    const ladderProtocol = await ladderProtocolResponse.json();
    const ladderResult = await ladderResultResponse.json();
    const ninetyProtocol = await ninetyProtocolResponse.json();
    const ninetyResult = await ninetyResultResponse.json();
    const reversedProtocol = await reversedProtocolResponse.json();
    const reversedResult = await reversedResultResponse.json();
    const numericProtocol = await numericProtocolResponse.json();
    const numericResult = await numericResultResponse.json();
    const calibrationProtocol = await calibrationProtocolResponse.json();
    const calibrationResult = await calibrationResultResponse.json();
    const nonceProtocol = await nonceProtocolResponse.json();
    const nonceResult = await nonceResultResponse.json();
    const phraseProtocol = await phraseProtocolResponse.json();
    const phraseResult = await phraseResultResponse.json();
    const nliProtocol = await nliProtocolResponse.json();
    const nliResult = await nliResultResponse.json();
    const debertaProtocol = await debertaProtocolResponse.json();
    const debertaResult = await debertaResultResponse.json();
    const freshProtocol = await freshProtocolResponse.json();
    const freshResult = await freshResultResponse.json();
    const contextProtocol = await contextProtocolResponse.json();
    const contextResult = await contextResultResponse.json();
    let index = 0;
    const links = [
      {
    ...
    const levelNames = [
      localized({ en: "Two plain sentences", ru: "Два простых предложения" }),
      localized({ en: "SOURCE + ANSWER + TOOLS", ru: "SOURCE + ANSWER + TOOLS" }),
      localized({ en: "Question + metadata + short window", ru: "Вопрос + метаданные + короткий фрагмент" }),
      localized({ en: "Long window · fact at the end", ru: "Длинный фрагмент · факт в конце" }),
      localized({ en: "Full packet · fact in the middle", ru: "Полный пакет · факт в середине" }),
    ];
    const ladderRows = ladderResult.levels.map(level => {
      const pair = ladderResult.records.filter(record => record.level === level.level);
      const decisions = pair.map(record => {
        const picked = record.actual.decision;
        const score = Math.round(record.actual.scores[picked] * 100);
        return `<article class="${record.correct ? "is-correct" : "is-critical"}"><span>${escapeHTML(record.expected.toUpperCase())} ${localized({ en: "NEEDED", ru: "НУЖЕН" })}</span><strong>${record.correct ? "●" : "×"} QWEN: ${escapeHTML(picked.toUpperCase())}</strong><b>${score}%</b></article>`;
      }).join("");
      const prompts = pair.map(record => `<div><b>${escapeHTML(record.expected.toUpperCase())}</b><pre>${escapeHTML(record.prompt)}</pre></div>`).join("");
      return `<article class="e007-ladder-step ${level.passed ? "is-correct" : "is-critical"}"><header><span>${localized({ en: "LEVEL", ru: "СТУПЕНЬ" })} ${level.level + 1}</span><h3>${escapeHTML(levelNames[level.level])}</h3><b>${level.prompt_tokens} tokens · ${level.correct}/2</b></header><div class="e007-ladder-decisions">${decisions}</div><details><summary>${localized({ en: "SHOW BOTH FULL PROMPTS", ru: "ПОКАЗАТЬ ОБА ПОЛНЫХ PROMPT" })}</summary>${prompts}</details></article>`;
    }).join("");
    const ninetyCards = ninetyResult.records.map(record => {
      const picked = record.decision;
      const pickedScore = Math.round(record.scores[picked] * 100);
      return `<article class="e007-ninety-card ${record.correct ? "is-correct" : "is-critical"}"><header><span>${escapeHTML(record.id)} · ${escapeHTML(record.domain.toUpperCase())} · ${record.prompt_words} ${localized({ en: "WORDS", ru: "СЛОВ" })}</span><strong>${record.correct ? "●" : "×"} QWEN: ${escapeHTML(picked.toUpperCase())} · ${pickedScore}%</strong></header><div><b>QUESTION</b><p>${escapeHTML(record.question)}</p></div><div><b>SOURCE</b><p>${escapeHTML(record.source)}</p></div><div><b>PROPOSED ANSWER</b><p>${escapeHTML(record.answer)}</p></div><footer>${localized({ en: "RIGHT BUTTON", ru: "ПРАВИЛЬНАЯ КНОПКА" })}: <strong>${escapeHTML(record.expected.toUpperCase())}</strong> · approve ${Math.round(record.scores.approve * 100)}% / reject ${Math.round(record.scores.reject * 100)}%</footer></article>`;
    }).join("");
    const reversedRows = reversedResult.records.map(record => `<tr class="${record.correct ? "is-correct" : "is-critical"}"><th>${escapeHTML(record.id)}<small>${escapeHTML(record.expected.toUpperCase())} ${localized({ en: "NEEDED", ru: "НУЖЕН" })}</small></th><td>${escapeHTML(record.previous_decision.toUpperCase())}</td><td><strong>${escapeHTML(record.decision.toUpperCase())}</strong><small>reject ${Math.round(record.scores.reject * 100)}% · approve ${Math.round(record.scores.approve * 100)}%</small></td><td>${record.correct ? `● ${localized({ en: "RIGHT", ru: "ВЕРНО" })}` : `× ${localized({ en: "WRONG", ru: "НЕВЕРНО" })}`}</td></tr>`).join("");
    const numericByKey = new Map(numericResult.records.map(record => [`${record.case_id}-${record.deck}`, record]));
    const numericRows = numericResult.pairs.map(pair => {
      const x = numericByKey.get(`${pair.case_id}-X`);
      const y = numericByKey.get(`${pair.case_id}-Y`);
      const xRight = x.semantic_correct;
      const yRight = y.semantic_correct;
      return `<tr class="${xRight && yRight ? "is-correct" : "is-critical"}"><th>${escapeHTML(pair.case_id)}<small>${localized({ en: "NEEDS", ru: "НУЖНО" })}: ${escapeHTML(x.expected_semantic.toUpperCase())}</small></th><td><strong>${escapeHTML(x.decision)}</strong> → ${escapeHTML(x.actual_semantic.toUpperCase())}<small>${xRight ? `● ${localized({ en: "right", ru: "верно" })}` : `× ${localized({ en: "wrong", ru: "неверно" })}`}</small></td><td><strong>${escapeHTML(y.decision)}</strong> → ${escapeHTML(y.actual_semantic.toUpperCase())}<small>${yRight ? `● ${localized({ en: "right", ru: "верно" })}` : `× ${localized({ en: "wrong", ru: "неверно" })}`}</small></td><td>${pair.label_flipped ? `● ${localized({ en: "YES", ru: "ДА" })}` : `× ${localized({ en: "NO", ru: "НЕТ" })}`}</td></tr>`;
    }).join("");
    const calibrationRows = calibrationResult.records.map(record => `<tr class="${record.agreement_method.correct && record.logit_calibration.correct ? "is-correct" : "is-critical"}"><th>${escapeHTML(record.case_id)}<small>${localized({ en: "NEEDS", ru: "НУЖНО" })}: ${escapeHTML(record.expected.toUpperCase())}</small></th><td><strong>${escapeHTML(record.agreement_method.decision.toUpperCase())}</strong><small>${record.agreement_method.answered ? (record.agreement_method.correct ? `● ${localized({ en: "right", ru: "верно" })}` : `× ${localized({ en: "wrong", ru: "неверно" })}`) : localized({ en: "deferred", ru: "отложено" })}</small></td><td><strong>${escapeHTML(record.logit_calibration.decision.toUpperCase())}</strong><small>${record.logit_calibration.correct ? `● ${localized({ en: "right", ru: "верно" })}` : `× ${localized({ en: "wrong", ru: "неверно" })}`} · margin ${record.logit_calibration.semantic_margin.toFixed(2)}</small></td></tr>`).join("");
    const phraseRows = phraseResult.families.map(family => `<tr class="${family.passed ? "is-correct" : "is-critical"}"><th>${escapeHTML(family.family_id)}<small>${escapeHTML(family.positive)} / ${escapeHTML(family.negative)}</small></th><td>${family.order_correct.POSITIVE_FIRST} / 10</td><td>${family.order_correct.NEGATIVE_FIRST} / 10</td><td><strong>${family.semantic_correct} / 20</strong></td><td>${family.order_stable_pairs} / 10</td><td>${family.passed ? `● ${localized({ en: "PASS", ru: "ПРОШЁЛ" })}` : `× ${localized({ en: "FAILED", ru: "НЕ ПРОШЁЛ" })}`}</td></tr>`).join("");
    const phraseMarkup = `<section id="phrase-length-control" class="e007-ninety e007-order-control"><div class="flow-step">CHECKPOINT 3C.6M · ${localized({ en: "ONE TO FOUR WORD BUTTONS", ru: "КНОПКИ ОТ ОДНОГО ДО ЧЕТЫРЁХ СЛОВ" })}</div><h2>${localized({ en: "Longer did not mean smarter.", ru: "Длиннее — не значит понятнее." })}</h2><p>${localized({ en: "This is not literal text search. Qwen had to decide whether SOURCE supports PROPOSED ANSWER. We scored each complete button phrase and repeated every case with the button order reversed.", ru: "Это не буквальный поиск строки. Qwen должна была решить, подтверждает ли ИСТОЧНИК ПРЕДЛОЖЕННЫЙ ОТВЕТ. Мы оценивали полную фразу на кнопке и повторяли каждый случай с обратным порядком кнопок." })}</p><div class="e007-result-metrics"><article class="is-critical"><span>include / not presented</span><strong>15 / 20</strong><small>${localized({ en: "diagnostic only · order-sensitive", ru: "только диагностика · зависит от порядка" })}</small></article><article class="is-critical"><span>${localized({ en: "BEST BALANCED PAIR", ru: "ЛУЧШАЯ РАВНАЯ ПАРА" })}</span><strong>11 / 20</strong><small>source supports answer / source lacks support</small></article><article class="is-critical"><span>${localized({ en: "PASSED", ru: "ПРОШЛИ GATE" })}</span><strong>0 / 4</strong><small>${localized({ en: "balanced families", ru: "равные пары" })}</small></article></div><div class="table-scroll"><table class="data-table e007-order-table"><thead><tr><th>${localized({ en: "BUTTONS", ru: "КНОПКИ" })}</th><th>${localized({ en: "POSITIVE FIRST", ru: "ПОЛОЖИТЕЛЬНАЯ ПЕРВОЙ" })}</th><th>${localized({ en: "NEGATIVE FIRST", ru: "ОТРИЦАТЕЛЬНАЯ ПЕРВОЙ" })}</th><th>${localized({ en: "TOTAL", ru: "ИТОГО" })}</th><th>${localized({ en: "SAME MEANING", ru: "СМЫСЛ НЕ СМЕНИЛСЯ" })}</th><th>GATE</th></tr></thead><tbody>${phraseRows}</tbody></table></div><p class="control-warning">${escapeHTML(localized(phraseProtocol.claim_boundary))}</p><div class="actions"><a class="quiet-link" href="/experiments/E007/phrase-length-result-v0.1.json">${localized({ en: "ALL 100 DECISIONS", ru: "ВСЕ 100 РЕШЕНИЙ" })} ↗</a><a class="quiet-link" href="/experiments/E007/phrase-length-world-v0.1.json">${localized({ en: "FROZEN PROMPTS", ru: "ЗАМОРОЖЕННЫЕ PROMPT" })} ↗</a><a class="quiet-link" href="/experiments/E007/phrase-length-protocol-v0.1.json">${localized({ en: "PLAN BEFORE RUN", ru: "ПЛАН ДО ЗАПУСКА" })} ↗</a></div></section>`;
    const nliLabel = value => ({ entailment: localized({ en: "SUPPORTED", ru: "ПОДТВЕРЖДЕНО" }), contradiction: localized({ en: "CONTRADICTED", ru: "ПРОТИВОРЕЧИТ" }), neutral: localized({ en: "NOT PROVEN", ru: "НЕ ДОКАЗАНО" }) }[value] || value);
    const nliRows = nliResult.records.map(record => `<tr class="${record.correct ? "is-correct" : "is-critical"}"><th>${escapeHTML(record.id)}<small>${localized({ en: "SOURCE", ru: "ИСТОЧНИК" })}: ${escapeHTML(record.premise)}</small></th><td>${escapeHTML(record.hypothesis)}</td><td>${escapeHTML(nliLabel(record.expected))}</td><td><strong>${record.correct ? "●" : "×"} ${escapeHTML(nliLabel(record.decision))}</strong><small>${Math.round(record.probabilities[record.decision] * 100)}%</small></td></tr>`).join("");
    const nliMarkup = `<section id="nli-minilm-control" class="e007-ninety e007-order-control"><div class="flow-step">CHECKPOINT 3C.6N · MINILM NLI</div><h2>${localized({ en: "Better than buttons, but not yet safe.", ru: "Лучше кнопок, но для приёмки пока опасно." })}</h2><p>${localized({ en: "A specialised 82M-parameter model received only an exact source passage and one claim. It had to choose: supported, contradicted, or not proven. No prompt buttons, examples, training, or threshold tuning were used.", ru: "Специализированная модель на 82 млн параметров получала только точный фрагмент источника и одно утверждение. Она выбирала: подтверждено, противоречит или не доказано. Без prompt-кнопок, примеров, обучения и настройки порога." })}</p><div class="e007-result-metrics"><article class="is-critical"><span>${localized({ en: "RIGHT", ru: "ВЕРНО" })}</span><strong>${nliResult.summary.correct} / ${nliResult.summary.total}</strong></article><article class="is-critical"><span>${localized({ en: "MISSED", ru: "ОШИБОК" })}</span><strong>${nliResult.summary.total - nliResult.summary.correct}</strong><small>${localized({ en: "one in every class", ru: "по одной в каждом классе" })}</small></article><article class="is-critical"><span>GATE</span><strong>${nliResult.summary.passed_locked_development_gate ? localized({ en: "PASSED", ru: "ПРОШЁЛ" }) : localized({ en: "FAILED", ru: "НЕ ПРОШЁЛ" })}</strong></article></div><div class="table-scroll"><table class="data-table e007-order-table"><thead><tr><th>${localized({ en: "EXACT SOURCE", ru: "ТОЧНЫЙ ИСТОЧНИК" })}</th><th>${localized({ en: "POCKET I CLAIM", ru: "УТВЕРЖДЕНИЕ POCKET I" })}</th><th>${localized({ en: "EXPECTED", ru: "ОЖИДАЛИ" })}</th><th>MINILM</th></tr></thead><tbody>${nliRows}</tbody></table></div><p class="control-warning">${escapeHTML(localized(nliProtocol.claim_boundary))}</p><div class="actions"><a class="quiet-link" href="/experiments/E007/nli-minilm-result-v0.1.json">${localized({ en: "ALL SCORES", ru: "ВСЕ ОЦЕНКИ" })} ↗</a><a class="quiet-link" href="/experiments/E007/nli-minilm-world-v0.1.json">${localized({ en: "FROZEN TEN", ru: "ЗАМОРОЖЕННЫЕ 10" })} ↗</a><a class="quiet-link" href="/experiments/E007/nli-minilm-protocol-v0.1.json">${localized({ en: "PLAN BEFORE RUN", ru: "ПЛАН ДО ЗАПУСКА" })} ↗</a></div></section>`;
    const debertaRows = debertaResult.records.map(record => `<tr class="${record.correct ? "is-correct" : "is-critical"}"><th>${escapeHTML(record.id)}<small>${escapeHTML(record.hypothesis)}</small></th><td>${record.minilm_correct ? "●" : "×"} ${escapeHTML(nliLabel(record.minilm_decision))}</td><td><strong>${record.correct ? "●" : "×"} ${escapeHTML(nliLabel(record.decision))}</strong><small>${Math.round(record.probabilities[record.decision] * 100)}%</small></td><td>${escapeHTML(nliLabel(record.expected))}</td></tr>`).join("");
    const debertaMarkup = `<section id="nli-deberta-control" class="e007-ninety e007-order-control"><div class="flow-step">CHECKPOINT 3C.6O · DEBERTA NLI</div><h2>${localized({ en: "Stronger helped, but did not solve it.", ru: "Более сильная помогла, но задачу не решила." })}</h2><p>${localized({ en: "DeBERTa fixed two MiniLM errors, introduced one new error, and moved the score from 7/10 to 8/10. Both models missed the technical idempotency paraphrase.", ru: "DeBERTa исправила две ошибки MiniLM, добавила одну новую и подняла результат с 7/10 до 8/10. Обе модели не поняли технический пересказ про идемпотентность." })}</p><div class="e007-result-metrics"><article class="is-critical"><span>MINILM · 82M</span><strong>7 / 10</strong></article><article class="is-critical"><span>DEBERTA · ~184M</span><strong>${debertaResult.summary.correct} / ${debertaResult.summary.total}</strong></article><article class="is-critical"><span>GATE</span><strong>${debertaResult.summary.passed_locked_development_gate ? localized({ en: "PASSED", ru: "ПРОШЁЛ" }) : localized({ en: "FAILED", ru: "НЕ ПРОШЁЛ" })}</strong></article></div><div class="table-scroll"><table class="data-table e007-order-table"><thead><tr><th>${localized({ en: "POCKET I CLAIM", ru: "УТВЕРЖДЕНИЕ POCKET I" })}</th><th>MINILM</th><th>DEBERTA</th><th>${localized({ en: "EXPECTED", ru: "ОЖИДАЛИ" })}</th></tr></thead><tbody>${debertaRows}</tbody></table></div><p class="control-warning">${escapeHTML(localized(debertaProtocol.claim_boundary))}</p><div class="actions"><a class="quiet-link" href="/experiments/E007/nli-deberta-result-v0.1.json">${localized({ en: "ALL SCORES", ru: "ВСЕ ОЦЕНКИ" })} ↗</a><a class="quiet-link" href="/experiments/E007/nli-deberta-protocol-v0.1.json">${localized({ en: "PLAN BEFORE RUN", ru: "ПЛАН ДО ЗАПУСКА" })} ↗</a></div></section>`;
    const freshRows = contextResult.records.map(record => `<tr class="${record.correct ? "is-correct" : "is-critical"}"><th>${escapeHTML(record.id)}<small>${localized({ en: "EVIDENCE", ru: "ДОКАЗАТЕЛЬСТВО" })}: ${escapeHTML(record.premise)}</small></th><td>${escapeHTML(record.hypothesis)}<small>${escapeHTML(record.gold_reason)}</small></td><td>${escapeHTML(nliLabel(record.expected))}</td><td>● ${escapeHTML(nliLabel(record.short_decision))}</td><td><strong>${record.correct ? "●" : "×"} ${escapeHTML(nliLabel(record.decision))}</strong><small>${Math.round(record.probabilities[record.decision] * 100)}% · ${record.input_tokens} tokens</small></td></tr>`).join("");
    const freshMarkup = `<section id="nli-fresh20-control" class="e007-ninety e007-order-control"><div class="flow-step">CHECKPOINT 3C.6P–Q · 20 ${localized({ en: "NEW CASES", ru: "НОВЫХ СЛУЧАЕВ" })}</div><h2>${localized({ en: "Short evidence: 20/20. Mixed context package: 15/20.", ru: "Короткое доказательство: 20/20. Смешанный пакет контекста: 15/20." })}</h2><p>${localized({ en: "Test 1 used twenty fresh short source-claim pairs. Test 2 kept every claim, evidence sentence, and gold label, then added the user question and surrounding source sentences. A manual audit confirmed all gold labels and all five new errors.", ru: "В Test 1 использованы двадцать новых коротких пар «источник — утверждение». В Test 2 сохранились все утверждения, доказательства и правильные классы, но добавились вопрос пользователя и соседние предложения источника. Ручная проверка подтвердила все эталоны и все пять новых ошибок." })}</p><div class="e007-result-metrics"><article class="is-correct"><span>TEST 1 · ${localized({ en: "SHORT", ru: "КОРОТКИЙ" })}</span><strong>${freshResult.summary.correct} / ${freshResult.summary.total}</strong><small>GATE ${freshResult.summary.passed_locked_development_gate ? "PASS" : "FAIL"}</small></article><article class="is-critical"><span>TEST 2 · ${localized({ en: "MORE CONTEXT", ru: "БОЛЬШЕ КОНТЕКСТА" })}</span><strong>${contextResult.summary.correct} / ${contextResult.summary.total}</strong><small>GATE ${contextResult.summary.passed_locked_development_gate ? "PASS" : "FAIL"}</small></article><article class="is-critical"><span>${localized({ en: "DECISIONS BROKEN", ru: "РЕШЕНИЙ СЛОМАЛОСЬ" })}</span><strong>${contextResult.summary.changed_decisions}</strong><small>${localized({ en: "all five became wrong", ru: "все пять стали неверными" })}</small></article></div><p class="control-warning">${localized({ en: "Important: Test 2 changed two things together — surrounding source text and the user's question. It shows that this combined package is unsafe, but does not yet tell us which addition caused each error.", ru: "Важно: Test 2 изменил сразу две вещи — соседний текст источника и вопрос пользователя. Мы знаем, что такой общий пакет опасен, но пока не знаем, какая именно добавка вызвала каждую ошибку." })}</p><div class="table-scroll"><table class="data-table e007-order-table"><thead><tr><th>${localized({ en: "CASE + EXACT EVIDENCE", ru: "СЛУЧАЙ + ТОЧНОЕ ДОКАЗАТЕЛЬСТВО" })}</th><th>${localized({ en: "CLAIM + HUMAN REASON", ru: "УТВЕРЖДЕНИЕ + ПРИЧИНА ЧЕЛОВЕКА" })}</th><th>${localized({ en: "GOLD", ru: "ЭТАЛОН" })}</th><th>${localized({ en: "SHORT", ru: "КОРОТКО" })}</th><th>${localized({ en: "WITH CONTEXT", ru: "С КОНТЕКСТОМ" })}</th></tr></thead><tbody>${freshRows}</tbody></table></div><p class="control-warning">${escapeHTML(localized(contextProtocol.claim_boundary))}</p><div class="actions"><a class="quiet-link" href="/experiments/E007/nli-fresh20-short-result-v0.1.json">TEST 1 · JSON ↗</a><a class="quiet-link" href="/experiments/E007/nli-fresh20-context-result-v0.1.json">TEST 2 · JSON ↗</a><a class="quiet-link" href="/experiments/E007/nli-fresh20-short-world-v0.1.json">${localized({ en: "FROZEN 20 + HUMAN REASONS", ru: "20 ЭТАЛОНОВ + ПРИЧИНЫ ЧЕЛОВЕКА" })} ↗</a><a class="quiet-link" href="/experiments/E007/nli-fresh20-context-world-v0.1.json">${localized({ en: "FROZEN CONTEXT", ru: "ЗАМОРОЖЕННЫЙ КОНТЕКСТ" })} ↗</a></div></section>`;
    const acceptedNliMarkup = `<section id="accepted-nli-module" class="e007-ninety e007-order-control"><div class="flow-step">${localized({ en: "ACCEPTED INTO HARNESS V0.2", ru: "ПРИНЯТО В HARNESS V0.2" })}</div><h2>${localized({ en: "One source passage. One small claim. Three outcomes.", ru: "Один фрагмент источника. Одно маленькое утверждение. Три исхода." })}</h2><p><code>${localized({ en: "exact verified passage + atomic claim", ru: "точный проверенный фрагмент + простое утверждение" })}</code> → DeBERTa NLI → <code>SUPPORTED / CONTRADICTED / NOT PROVEN</code></p><p class="control-warning">${localized({ en: "The user's question stays outside this call. This module checks only whether the passage supports the claim. It does not prove that the source is true, current, independent, applicable, or safe to share.", ru: "Вопрос пользователя остаётся снаружи этого вызова. Модуль проверяет только одно: подтверждает ли фрагмент утверждение. Он не доказывает, что источник правдив, свеж, независим, применим или безопасен для отправки." })}</p><div class="actions"><a class="button secondary" href="/experiments/E007/harness-v0.2.md">${localized({ en: "OPEN THE ACCEPTED HARNESS", ru: "ОТКРЫТЬ ПРИНЯТЫЙ HARNESS" })} ↗</a></div></section>`;
    const nonceByKey = new Map(nonceResult.records.map(record => [`${record.case_id}-${record.deck}`, record]));
    const nonceRows = nonceResult.pairs.map(pair => {
      const main = nonceByKey.get(`${pair.case_id}-MAIN`);
      const mirror = nonceByKey.get(`${pair.case_id}-MIRROR`);
      return `<tr class="${main.semantic_correct && mirror.semantic_correct ? "is-correct" : "is-critical"}"><th>${escapeHTML(pair.case_id)}<small>${localized({ en: "NEEDS", ru: "НУЖНО" })}: ${escapeHTML(main.expected_semantic.toUpperCase())}</small></th><td><strong>${escapeHTML(main.decision)}</strong> → ${escapeHTML(main.actual_semantic.toUpperCase())}<small>${main.semantic_correct ? `● ${localized({ en: "right", ru: "верно" })}` : `× ${localized({ en: "wrong", ru: "неверно" })}`}</small></td><td><strong>${escapeHTML(mirror.decision)}</strong> → ${escapeHTML(mirror.actual_semantic.toUpperCase())}<small>${mirror.semantic_correct ? `● ${localized({ en: "right", ru: "верно" })}` : `× ${localized({ en: "wrong", ru: "неверно" })}`}</small></td><td>${pair.label_flipped ? `● ${localized({ en: "YES", ru: "ДА" })}` : `× ${localized({ en: "NO", ru: "НЕТ" })}`}</td></tr>`;
    }).join("");
    target.querySelector(".experiment-loading").outerHTML = `<section id="nonce-word-control" class="e007-ninety e007-order-control"><div class="flow-step">CHECKPOINT 3C.6L · KSEL / PTHY</div><h2>${localized({ en: "Invented words did not make it reason.", ru: "Выдуманные слова не заставили её рассуждать." })}</h2><p>${localized({ en: "MAIN hides approve behind KSEL. Qwen chose KSEL on all ten. MIRROR swaps the meanings but scored only 3/10. Across both decks it understood 8/20.", ru: "MAIN прячет approve за KSEL. Qwen выбрала KSEL во всех десяти случаях. MIRROR меняет смыслы местами, но набрала лишь 3/10. В двух колодах вместе модель поняла 8/20." })}</p><div class="e007-result-metrics"><article class="is-critical"><span>MAIN</span><strong>5 / 10</strong><small>10 × KSEL</small></article><article class="is-critical"><span>MIRROR</span><strong>3 / 10</strong></article><article class="is-critical"><span>${localized({ en: "MEANING RIGHT", ru: "СМЫСЛ ВЕРЕН" })}</span><strong>8 / 20</strong></article></div><p class="e007-deck-key"><b>MAIN</b> · KSEL = approve · PTHY = reject<br><b>MIRROR</b> · KSEL = reject · PTHY = approve<br>${localized({ en: "Both complete words contain two tokens; the scorer adds both token probabilities.", ru: "Оба полных слова состоят из двух токенов; scorer складывает вероятности обеих частей." })}</p><div class="table-scroll"><table class="data-table e007-order-table"><thead><tr><th>${localized({ en: "CASE", ru: "СЛУЧАЙ" })}</th><th>MAIN</th><th>MIRROR</th><th>${localized({ en: "LABEL FLIPPED?", ru: "ЯРЛЫК СМЕНИЛСЯ?" })}</th></tr></thead><tbody>${nonceRows}</tbody></table></div><p class="control-warning">${escapeHTML(localized(nonceProtocol.claim_boundary))}</p><div class="actions"><a class="quiet-link" href="/experiments/E007/nonce-word-result-v0.1.json">${localized({ en: "ALL SCORES", ru: "ВСЕ ОЦЕНКИ" })} ↗</a><a class="quiet-link" href="/experiments/E007/nonce-word-world-v0.1.json">${localized({ en: "FROZEN PROMPTS", ru: "ЗАМОРОЖЕННЫЕ PROMPT" })} ↗</a></div></section><section id="mirrored-calibration" class="e007-ninety e007-order-control"><div class="flow-step">CHECKPOINT 3C.6K · ${localized({ en: "TRY TO REMOVE SYMBOL BIAS", ru: "ПРОБУЕМ УБРАТЬ ЛЮБОВЬ К СИМВОЛУ" })}</div><h2>${localized({ en: "It did not work.", ru: "Не сработало." })}</h2><p>${localized({ en: "Method 1 answered only when both mirrored decks meant the same thing. Method 2 mathematically subtracted the 1/A preference. Neither produced a reliable judge.", ru: "Метод 1 отвечал только когда обе зеркальные колоды имели один смысл. Метод 2 математически вычитал предпочтение 1/A. Ни один не дал надёжного судью." })}</p><div class="e007-result-metrics"><article class="is-critical"><span>${localized({ en: "AGREEMENT ANSWERED", ru: "СОГЛАСИЕ ОТВЕТИЛО" })}</span><strong>4 / 10</strong><small>${localized({ en: "2 right · 2 wrong", ru: "2 верно · 2 неверно" })}</small></article><article class="is-critical"><span>${localized({ en: "AGREEMENT UNSURE", ru: "СОГЛАСИЕ НЕ УВЕРЕНО" })}</span><strong>6 / 10</strong></article><article class="is-critical"><span>${localized({ en: "MATH RIGHT", ru: "МАТЕМАТИКА ВЕРНА" })}</span><strong>5 / 10</strong><small>10 × approve</small></article></div><div class="table-scroll"><table class="data-table e007-order-table"><thead><tr><th>${localized({ en: "CASE", ru: "СЛУЧАЙ" })}</th><th>${localized({ en: "ONLY IF BOTH AGREE", ru: "ТОЛЬКО ПРИ СОГЛАСИИ" })}</th><th>${localized({ en: "AFTER SUBTRACTION", ru: "ПОСЛЕ ВЫЧИТАНИЯ" })}</th></tr></thead><tbody>${calibrationRows}</tbody></table></div><p class="control-warning">${escapeHTML(localized(calibrationProtocol.scientific_status))}</p><p class="control-warning">${escapeHTML(localized(calibrationProtocol.claim_boundary))}</p><div class="actions"><a class="quiet-link" href="/experiments/E007/mirrored-calibration-result-v0.1.json">${localized({ en: "RAW CALCULATION", ru: "ВСЕ РАСЧЁТЫ" })} ↗</a></div></section><section id="numeric-letter-control" class="e007-ninety e007-order-control"><div class="flow-step">CHECKPOINT 3C.6J · ${localized({ en: "MIRRORED 1 / A", ru: "ЗЕРКАЛЬНЫЕ 1 / A" })}</div><h2>${localized({ en: "Now it prefers A.", ru: "Теперь она предпочитает A." })}</h2><p>${localized({ en: "Deck X says 1=approve and A=reject. Deck Y swaps the meanings. Qwen chose A 16/20 times and changed symbols in only 4/10 pairs. Neutral labels moved the bias; they did not create a semantic judge.", ru: "В колоде X: 1=approve, A=reject. В колоде Y значения меняются местами. Qwen выбрала A в 16/20 случаев и поменяла символ лишь в 4/10 пар. Нейтральные ярлыки перенесли смещение, но не создали смыслового судью." })}</p><div class="e007-result-metrics"><article class="is-critical"><span>A ${localized({ en: "CHOSEN", ru: "ВЫБРАНА" })}</span><strong>16 / 20</strong></article><article class="is-critical"><span>${localized({ en: "PAIRS FLIPPED", ru: "СИМВОЛ СМЕНИЛСЯ" })}</span><strong>4 / 10</strong></article><article class="is-critical"><span>${localized({ en: "MEANING RIGHT", ru: "СМЫСЛ ВЕРЕН" })}</span><strong>10 / 20</strong></article></div><p class="e007-deck-key"><b>DECK X</b> · 1 = approve · A = reject<br><b>DECK Y</b> · 1 = reject · A = approve</p><div class="table-scroll"><table class="data-table e007-order-table"><thead><tr><th>${localized({ en: "CASE", ru: "СЛУЧАЙ" })}</th><th>DECK X</th><th>DECK Y</th><th>${localized({ en: "SYMBOL FLIPPED?", ru: "СИМВОЛ СМЕНИЛСЯ?" })}</th></tr></thead><tbody>${numericRows}</tbody></table></div><p class="control-warning">${escapeHTML(localized(numericProtocol.claim_boundary))}</p><div class="actions"><a class="quiet-link" href="/experiments/E007/numeric-letter-result-v0.1.json">${localized({ en: "RAW 20 DECISIONS", ru: "ВСЕ 20 РЕШЕНИЙ" })} ↗</a><a class="quiet-link" href="/experiments/E007/numeric-letter-world-v0.1.json">${localized({ en: "FROZEN TWO DECKS", ru: "ДВЕ ЗАМОРОЖЕННЫЕ КОЛОДЫ" })} ↗</a></div></section><section id="reversed-button-control" class="e007-ninety e007-order-control"><div class="flow-step">CHECKPOINT 3C.6I · ${localized({ en: "REJECT MOVED FIRST", ru: "REJECT ПОСТАВИЛИ ПЕРВЫМ" })}</div><h2>${localized({ en: "Nothing changed.", ru: "Ничего не изменилось." })}</h2><p>${localized({ en: "Before: 10 APPROVE. After putting REJECT first: still 10 APPROVE. This is not a simple first-option effect; Qwen strongly prefers the approve label or its meaning in this gate.", ru: "До перестановки: 10 APPROVE. После того как REJECT поставили первым: снова 10 APPROVE. Это не простое предпочтение первого варианта; в этом gate Qwen сильно предпочитает слово approve или связанный с ним смысл." })}</p><div class="e007-result-metrics"><article class="is-critical"><span>${localized({ en: "CHOICES CHANGED", ru: "РЕШЕНИЙ ИЗМЕНИЛОСЬ" })}</span><strong>0 / 10</strong></article><article class="is-critical"><span>REJECT ${localized({ en: "CHOSEN", ru: "ВЫБРАН" })}</span><strong>0 / 10</strong></article><article class="is-critical"><span>${localized({ en: "RIGHT", ru: "ВЕРНО" })}</span><strong>5 / 10</strong></article></div><div class="table-scroll"><table class="data-table e007-order-table"><thead><tr><th>${localized({ en: "CASE", ru: "СЛУЧАЙ" })}</th><th>APPROVE ${localized({ en: "FIRST", ru: "ПЕРВЫМ" })}</th><th>REJECT ${localized({ en: "FIRST", ru: "ПЕРВЫМ" })}</th><th>${localized({ en: "RESULT", ru: "ИТОГ" })}</th></tr></thead><tbody>${reversedRows}</tbody></table></div><p class="control-warning">${escapeHTML(localized(reversedProtocol.claim_boundary))}</p><div class="actions"><a class="quiet-link" href="/experiments/E007/ninety-word-reversed-result-v0.1.json">${localized({ en: "RAW PAIRED RESULT", ru: "СЫРОЕ ПАРНОЕ СРАВНЕНИЕ" })} ↗</a><a class="quiet-link" href="/experiments/E007/ninety-word-reversed-world-v0.1.json">${localized({ en: "FROZEN REVERSED PROMPTS", ru: "ЗАМОРОЖЕННЫЕ ПЕРЕСТАВЛЕННЫЕ PROMPT" })} ↗</a></div></section><section id="ninety-word-gate" class="e007-ninety"><div class="flow-step">CHECKPOINT 3C.6H · ${localized({ en: "TEN COMPLETE PROMPTS UNDER 90 WORDS", ru: "ДЕСЯТЬ ПОЛНЫХ PROMPT ДО 90 СЛОВ" })}</div><h2>${localized({ en: "It approved all ten.", ru: "Она одобрила все десять." })}</h2><p>${localized({ en: "All five supported answers passed. All five unsupported or conflicting answers also passed. Shorter context did not remove the approve bias.", ru: "Все пять подтверждённых ответов прошли. Но также прошли все пять неподтверждённых или противоречащих ответов. Короткий контекст не убрал смещение к approve." })}</p><div class="e007-result-metrics"><article class="is-correct"><span>APPROVE ${localized({ en: "CASES", ru: "СЛУЧАИ" })}</span><strong>5 / 5</strong></article><article class="is-critical"><span>REJECT ${localized({ en: "CASES", ru: "СЛУЧАИ" })}</span><strong>0 / 5</strong></article><article class="is-critical"><span>${localized({ en: "TOTAL", ru: "ИТОГО" })}</span><strong>5 / 10</strong></article></div><div class="e007-ninety-grid">${ninetyCards}</div><p class="control-warning">${escapeHTML(localized(ninetyProtocol.claim_boundary))}</p><div class="actions"><a class="quiet-link" href="/experiments/E007/ninety-word-result-v0.1.json">${localized({ en: "RAW RESULT", ru: "СЫРОЙ РЕЗУЛЬТАТ" })} ↗</a><a class="quiet-link" href="/experiments/E007/ninety-word-world-v0.1.json">${localized({ en: "FROZEN PROMPTS", ru: "ЗАМОРОЖЕННЫЕ PROMPT" })} ↗</a></div></section><section id="context-ladder" class="e007-context-ladder"><div class="flow-step">CHECKPOINT 3C.6G · ${localized({ en: "CONTEXT LADDER", ru: "ЛЕСТНИЦА КОНТЕКСТА" })}</div><h2>${localized({ en: "It holds through 145 tokens. It breaks at 513.", ru: "До 145 токенов держится. На 513 ломается." })}</h2><p>${localized({ en: "Every pair asks the same thing. ACCEPT says the source is red; REJECT says it is blue. Only the wrapper grows.", ru: "Каждая пара спрашивает одно и то же. В ACCEPT источник говорит «красный», в REJECT — «синий». Растёт только упаковка." })}</p><div class="e007-ladder-list">${ladderRows}</div><p class="control-warning">${escapeHTML(localized(ladderProtocol.claim_boundary))}</p><div class="actions"><a class="quiet-link" href="/experiments/E007/context-ladder-result-v0.1.json">${localized({ en: "ALL TEN SCORES", ru: "ВСЕ ДЕСЯТЬ ОЦЕНОК" })} ↗</a><a class="quiet-link" href="/experiments/E007/context-ladder-world-v0.1.json">${localized({ en: "FROZEN PROMPTS", ru: "ЗАМОРОЖЕННЫЕ PROMPT" })} ↗</a></div></section><section id="button-sanity" class="e007-button-sanity"><div class="flow-step">CHECKPOINT 3C.6F · ${localized({ en: "CAN IT PRESS BOTH BUTTONS?", ru: "УМЕЕТ ЛИ ОНА НАЖИМАТЬ ОБЕ КНОПКИ?" })}</div><h2>${localized({ en: "Yes. On the easiest possible comparison: 2/2.", ru: "Да. На самом простом сравнении: 2/2." })}</h2><p>${localized({ en: "The first case needs ACCEPT. The second differs by one colour word and needs REJECT. Everything else stayed identical.", ru: "В первом случае нужен ACCEPT. Во втором изменилось одно слово цвета и нужен REJECT. Всё остальное осталось одинаковым." })}</p><div class="e007-sanity-cards">${sanityCards}</div><p class="control-warning">${escapeHTML(localized(sanityProtocol.claim_boundary))}</p></section><section class="e005-gate4-result-verdict is-failed"><span>CHECKPOINT 3C.6E · ${localized({ en: "TEN RICHER CASES", ru: "ДЕСЯТЬ БОЛЕЕ СЛОЖНЫХ СЛУЧАЕВ" })}</span><h2>${localized({ en: "Qwen pressed ACCEPT thirty times out of thirty.", ru: "Qwen нажала «ПРИНЯТЬ» тридцать раз из тридцати." })}</h2><p>${localized({ en: "It kept all three useful packets, but also let all seven traps through. The buttons work; richer meaning is where it fails.", ru: "Она сохранила все три полезных пакета, но также пропустила все семь ловушек. Кнопки работают; сбой начинается на более сложном смысле." })}</p></section><div class="e007-result-metrics"><article class="is-correct"><span>${localized({ en: "USEFUL KEPT", ru: "ПОЛЕЗНЫХ СОХРАНЕНО" })}</span><strong>3 / 3</strong></article><article class="is-critical"><span>${localized({ en: "TRAPS ACCEPTED", ru: "ЛОВУШЕК ПРИНЯТО" })}</span><strong>7 / 7</strong></article><article class="is-critical"><span>${localized({ en: "FINAL DECISIONS RIGHT", ru: "ИТОГОВ ВЕРНО" })}</span><strong>3 / 10</strong></article></div><div class="e007-ten-viewer"></div><p class="control-warning">${escapeHTML(localized(protocol.claim_boundary))}</p><div class="actions"><a class="button secondary" href="/experiment/e007/">${localized({ en: "BACK TO E007", ru: "НАЗАД В E007" })}</a><a class="quiet-link" href="/experiments/E007/button-sanity-result-v0.1.json">${localized({ en: "TWO RAW SCORES", ru: "ДВЕ СЫРЫЕ ОЦЕНКИ" })} ↗</a><a class="quiet-link" href="/experiments/E007/atomic-button-result-v0.1.json">${localized({ en: "ALL TEN RAW SCORES", ru: "ВСЕ ДЕСЯТЬ СЫРЫХ ОЦЕНОК" })} ↗</a></div>`;
    target.querySelector("#nonce-word-control")?.insertAdjacentHTML("beforebegin", phraseMarkup);
    target.querySelector("#phrase-length-control")?.insertAdjacentHTML("beforebegin", nliMarkup);
    target.querySelector("#nli-minilm-control")?.insertAdjacentHTML("beforebegin", debertaMarkup);
    target.querySelector("#nli-deberta-control")?.insertAdjacentHTML("beforebegin", acceptedNliMarkup + freshMarkup);
    renderCase();
  } catch (error) {
...
async function loadE007KnowledgeChain() {
  ...
  if (!target) return;
  try {
    const [protocolResponse, resultResponse, physicalResponse] = await Promise.all([
      ...
      fetch("/experiments/E007/knowledge-chain-physical-result-v0.2.json", { cache: "no-store" }),
    ]);
    if (!protocolResponse.ok || !resultResponse.ok || !physicalResponse.ok) throw new Error("Gate 12A data missing");
    const protocol = await protocolResponse.json();
    const result = await resultResponse.json();
    const physical = await physicalResponse.json();
    const decisionNames = {
    ...
    target.querySelector(".experiment-loading").outerHTML = `<section class="e007-chain-result"><section class="e007-physical-chain"><div class="flow-step">GATE 12A.2 · ${localized({ en: "TWO PHYSICAL MACHINES", ru: "ДВЕ ФИЗИЧЕСКИЕ МАШИНЫ" })}</div><h2>yukabox → miracle-prod</h2><div class="e007-result-metrics"><article class="is-correct"><span>SHA-256</span><strong>${physical.checks.payload_sha256_matches ? localized({ en: "MATCH", ru: "СОВПАЛ" }) : localized({ en: "DIFFERENT", ru: "НЕ СОВПАЛ" })}</strong></article><article class="is-correct"><span>${localized({ en: "CURRENT", ru: "СЕЙЧАС" })}</span><strong>${escapeHTML(physical.receiver_receipt.current_revision_id)}</strong></article><article class="is-correct"><span>${localized({ en: "HISTORY KEPT", ru: "ИСТОРИЯ СОХРАНЕНА" })}</span><strong>${physical.receiver_receipt.history_revision_ids.length} / 2</strong></article></div><div class="e007-chain-timeline"><article class="e007-chain-node is-history"><span>PHY-R1</span><strong>${localized({ en: "First belief", ru: "Первая мысль" })}</strong></article><b class="e007-chain-arrow">→</b><article class="e007-chain-node is-history"><span>PHY-R2</span><strong>${localized({ en: "Newer belief", ru: "Следующая мысль" })}</strong></article><b class="e007-chain-arrow">→</b><article class="e007-chain-node is-current"><span>PHY-R3 · ${localized({ en: "CURRENT", ru: "СЕЙЧАС" })}</span><strong>${escapeHTML(physical.receiver_receipt.current_claim)}</strong></article></div><p class="control-warning">${localized({ en: "The payload really moved between two machines. SSH was only the test carrier; it is not the future Pocket i network protocol.", ru: "Пакет действительно прошёл между двумя машинами. SSH здесь только транспорт теста, а не будущий сетевой протокол Pocket i." })}</p></section><div class="flow-step">GATE 12A.1 · ${localized({ en: "TEN SYNTHETIC CHAINS", ru: "10 СИНТЕТИЧЕСКИХ ЦЕПОЧЕК" })}</div><div class="e007-result-metrics"><article class="is-correct"><span>${localized({ en: "CHAINS UNDERSTOOD", ru: "ЦЕПОЧЕК ПОНЯТО" })}</span><strong>${result.summary.case_decisions_correct} / ${result.summary.total_cases}</strong></article><article class="is-correct"><span>${localized({ en: "PACKETS UNCHANGED", ru: "ПАКЕТОВ БЕЗ ИЗМЕНЕНИЙ" })}</span><strong>${result.summary.transport_roundtrips_exact} / ${result.summary.total_cases}</strong></article><article class="is-correct"><span>${localized({ en: "OLD RECORDS LOST", ru: "СТАРЫХ ЗАПИСЕЙ ПОТЕРЯНО" })}</span><strong>${result.summary.history_revisions_lost}</strong></article></div><p class="control-warning">${localized({ en: "The old relation words are now removed from the accepted candidate contract. The receiver checks only one chain and its current head.", ru: "Слова confirms/refines/replaces убраны из нового контракта. Приёмник проверяет только одну цепочку и её последнюю запись." })}</p><div class="e007-chain-list">${cards}</div><div class="actions"><a class="quiet-link" href="/experiments/E007/knowledge-chain-physical-result-v0.2.json">${localized({ en: "PHYSICAL RECEIPT", ru: "ФИЗИЧЕСКАЯ КВИТАНЦИЯ" })} ↗</a><a class="quiet-link" href="/experiments/E007/knowledge-chain-result-v0.1.json">${localized({ en: "ALL RAW RESULTS", ru: "ВСЕ СЫРЫЕ РЕЗУЛЬТАТЫ" })} ↗</a><a class="quiet-link" href="/experiments/E007/knowledge-chain-world-v0.1.json">${localized({ en: "FROZEN TEN CHAINS", ru: "10 ЗАМОРОЖЕННЫХ ЦЕПОЧЕК" })} ↗</a><a class="quiet-link" href="/experiments/E007/knowledge-chain-physical-protocol-v0.2.json">${localized({ en: "PHYSICAL PLAN BEFORE RUN", ru: "ПЛАН ФИЗИЧЕСКОГО ТЕСТА" })} ↗</a></div></section>`;
  } catch (error) {
...
async function loadE007AnswerPiles() {
  const target = document.querySelector(".e007-pile-page");
  if (!target) return;
  try {
    const [resultResponse, worldResponse] = await Promise.all([
      fetch("/experiments/E007/answer-piles-nli-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/answer-piles-nli-world-v0.1.json", { cache: "no-store" }),
    ]);
    if (!resultResponse.ok || !worldResponse.ok) throw new Error("Gate 13B data missing");
    const result = await resultResponse.json();
    const world = await worldResponse.json();
    const expectedSizes = new Map(world.gold_piles.map(pile => [pile.id, pile.answer_ids.length]));
    const relationName = value => ({
      entailment: localized({ en: "CONFIRMS", ru: "ПОДТВЕРЖДАЕТ" }),
      neutral: localized({ en: "DOES NOT PROVE", ru: "НЕ ДОКАЗЫВАЕТ" }),
      contradiction: localized({ en: "CALLS IT A CONFLICT", ru: "НАЗЫВАЕТ ПРОТИВОРЕЧИЕМ" }),
    }[value] || value);
    const piles = result.piles.map(pile => {
      const expected = expectedSizes.get(pile.gold_piles[0]) || 1;
      const complete = pile.gold_piles.length === 1 && pile.answers.length === expected;
      const split = pile.gold_piles.length === 1 && pile.answers.length < expected;
      const status = complete
        ? localized({ en: "CORRECT PILE", ru: "ПРАВИЛЬНАЯ СТОПКА" })
        : split
          ? localized({ en: "SHOULD HAVE A PARTNER", ru: "НЕ ХВАТАЕТ ПАРЫ" })
          : localized({ en: "WRONG MERGE", ru: "ОШИБОЧНАЯ СКЛЕЙКА" });
      const answers = pile.answers.map(answer => `<li><b>${escapeHTML(answer.id)} · ${escapeHTML(answer.pocket_id)}</b><span>${escapeHTML(answer.text)}</span></li>`).join("");
      return `<article class="e007-cluster-card ${complete ? "is-correct" : split ? "is-warning" : "is-critical"}"><header><span>${escapeHTML(pile.pile_id)}</span><strong>${escapeHTML(status)}</strong><small>${pile.answers.length} ${localized({ en: "answers", ru: "ответа" })} · ${localized({ en: "expected meaning", ru: "ожидаемый смысл" })} ${escapeHTML(pile.gold_piles.join(" + "))}</small></header><ul>${answers}</ul></article>`;
    }).join("");
    const missed = world.gold_piles.filter(pile => pile.answer_ids.length > 1).map(pile => {
      const pair = result.pair_records.find(record => pile.answer_ids.includes(record.left_id) && pile.answer_ids.includes(record.right_id));
      if (!pair || pair.relation === "same_version") return "";
      const leftScore = Math.round(pair.left_to_right.probabilities[pair.left_to_right.decision] * 100);
...
async function loadE007AnswerPilesQwenSandwich() {
  const target = document.querySelector(".e007-sandwich-page");
  if (!target) return;
  try {
    const [resultResponse, auditResponse, decisionResponse] = await Promise.all([
      fetch("/experiments/E007/answer-piles-qwen-sandwich-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/answer-piles-qwen-sandwich-human-audit-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/answer-piles-accepted-architecture-v0.1.json", { cache: "no-store" }),
    ]);
    if (!resultResponse.ok || !auditResponse.ok || !decisionResponse.ok) throw new Error("Gate 13D data missing");
    const result = await resultResponse.json();
    const audit = await auditResponse.json();
    const decision = await decisionResponse.json();
    const canonicalCards = result.canonical_records.map(record => {
      const worst = record.validation.reduce((current, check) => {
        const score = check.probabilities[check.decision];
        return !current || score < current.score ? { decision: check.decision, score } : current;
      }, null);
      return `<article class="e007-canonical-card ${record.valid ? "is-correct" : "is-critical"}"><header><b>${escapeHTML(record.pile_id)}</b><strong>${record.valid ? localized({ en: "PASSED CHECK", ru: "ПРОШЛА ПРОВЕРКУ" }) : localized({ en: "BLOCKED", ru: "ЗАБЛОКИРОВАНА" })}</strong></header><small>${localized({ en: "ORIGINAL PILE", ru: "ИСХОДНАЯ СТОПКА" })}</small><ul>${record.original_answers.map(answer => `<li>${escapeHTML(answer)}</li>`).join("")}</ul><small>QWEN 0.6B</small><p>${escapeHTML(record.canonical_claim || "—")}</p><small>${localized({ en: "WEAKEST DEBERTA CHECK", ru: "САМАЯ СЛАБАЯ ПРОВЕРКА DEBERTA" })}: ${escapeHTML(worst.decision)} · ${Math.round(worst.score * 100)}%</small></article>`;
    }).join("");
    const findingText = {
      "AUDIT-01": localized({ en: "Good block: Qwen changed ‘can indicate’ into certainty.", ru: "Хорошая блокировка: Qwen заменила «может указывать» на уверенное утверждение." }),
      "AUDIT-02": localized({ en: "Owner decision: losing ‘exclusively’ was acceptable in this particular instruction.", ru: "Решение владельца: потеря слова «только» допустима в этой конкретной инструкции." }),
      "AUDIT-03": localized({ en: "Fragile recovery: the deciding score was only 50.6% versus 49.4%.", ru: "Хрупкое возвращение: решающий перевес составил лишь 50,6% против 49,4%." }),
    };
    const findings = audit.findings.map(finding => `<article class="e007-audit-card ${finding.verdict === "correctly_blocked" || finding.id === "AUDIT-02" ? "is-correct" : "is-critical"}"><b>${escapeHTML(finding.id)}</b><strong>${escapeHTML(findingText[finding.id])}</strong><p>${escapeHTML(finding.reason)}</p></article>`).join("");
    const stepNames = {
      deberta_initial_piles: localized({ en: "CAUTIOUS FIRST PILES", ru: "ОСТОРОЖНЫЕ ПЕРВЫЕ СТОПКИ" }),
      qwen_canonical_claim: localized({ en: "ONE SHORT CLAIM", ru: "ОДНА КОРОТКАЯ МЫСЛЬ" }),
      deberta_rewrite_guard: localized({ en: "CHECK AGAINST ORIGINALS", ru: "ПРОВЕРКА ПО ИСХОДНИКАМ" }),
      deberta_final_piles: localized({ en: "FINAL VERSIONS", ru: "ИТОГОВЫЕ ВЕРСИИ" }),
    };
    const stepDescriptions = {
      deberta_initial_piles: localized({ en: "DeBERTa groups only mutually supporting original answers.", ru: "DeBERTa складывает вместе только взаимно подтверждающие исходные ответы." }),
      qwen_canonical_claim: localized({ en: "Qwen turns each pile into one readable sentence.", ru: "Qwen превращает каждую стопку в одну понятную фразу." }),
      deberta_rewrite_guard: localized({ en: "The new sentence must match every original in both directions. If not, we keep the originals.", ru: "Новая фраза должна совпасть по смыслу с каждым исходником в обе стороны. Не совпала — сохраняем исходники." }),
      deberta_final_piles: localized({ en: "Validated sentences are compared again. Different versions remain in different piles.", ru: "Проверенные фразы сравниваются снова. Разные версии остаются в разных стопках." }),
    };
    const pipeline = decision.architecture.map(step => `<article><span>${step.step}</span><strong>${escapeHTML(stepNames[step.module])}</strong><p>${escapeHTML(stepDescriptions[step.module])}</p></article>`).join("");
    target.querySelector(".experiment-loading").outerHTML = `<section class="e007-cluster-result"><div class="e005-gate4-result-verdict is-passed"><span>${localized({ en: "OWNER DECISION · ACCEPTED FOR HARNESS V0.2", ru: "РЕШЕНИЕ ВЛАДЕЛЬЦА · ПРИНЯТО ДЛЯ HARNESS V0.2" })}</span><h2>${localized({ en: "Keep the originals. Let Qwen simplify. Make DeBERTa check.", ru: "Храним исходники. Qwen упрощает. DeBERTa проверяет." })}</h2><p>${localized({ en: "This is the current MVP architecture. The experiment remains honestly limited: 5 of 6 piles, synthetic English data, and no scale test yet.", ru: "Это текущая архитектура MVP. Ограничения эксперимента сохраняются честно: 5 стопок из 6, синтетические английские данные и пока без проверки масштаба." })}</p></div><h2>${localized({ en: "The accepted four steps", ru: "Четыре принятых шага" })}</h2><div class="e007-accepted-pipeline">${pipeline}</div><p class="control-warning">${localized({ en: "Qwen never replaces the evidence. Originals, sources, and lineage always remain underneath the short claim.", ru: "Qwen никогда не заменяет доказательства. Исходные ответы, источники и происхождение всегда остаются под короткой фразой." })}</p><div class="e007-result-metrics"><article class="is-correct"><span>${localized({ en: "FINAL CORRECT PILES", ru: "ПРАВИЛЬНЫХ СТОПОК" })}</span><strong>${result.summary.final_exact_paraphrase_piles} / 6</strong></article><article class="is-correct"><span>${localized({ en: "FALSE MERGES", ru: "ЛОЖНЫХ СКЛЕЕК" })}</span><strong>${result.summary.false_merges}</strong></article><article class="needs-review"><span>${localized({ en: "VALIDATED REWRITES", ru: "ПРОВЕРЕННЫХ ПЕРЕПИСЫВАНИЙ" })}</span><strong>${result.summary.valid_canonical_claims} / 10</strong></article></div><div class="e007-audit-list">${findings}</div><h2>${localized({ en: "All ten Qwen rewrites", ru: "Все десять переписываний Qwen" })}</h2><div class="e007-canonical-list">${canonicalCards}</div><div class="actions"><a class="primary-link" href="/experiments/E007/answer-piles-accepted-architecture-v0.1.json">${localized({ en: "ACCEPTED ARCHITECTURE", ru: "ПРИНЯТАЯ АРХИТЕКТУРА" })} ↗</a><a class="quiet-link" href="/experiments/E007/answer-piles-qwen-sandwich-result-v0.1.json">${localized({ en: "ALL CHECKS", ru: "ВСЕ ПРОВЕРКИ" })} ↗</a><a class="quiet-link" href="/experiments/E007/answer-piles-qwen-sandwich-human-audit-v0.1.json">${localized({ en: "MANUAL AUDIT", ru: "РУЧНАЯ ПРОВЕРКА" })} ↗</a><a class="quiet-link" href="/experiments/E007/answer-piles-qwen-sandwich-invalid-preflight-v0.1.json">INVALID PREFLIGHT ↗</a><a class="quiet-link" href="/experiments/E007/answer-piles-qwen-sandwich-protocol-v0.1.json">${localized({ en: "PLAN BEFORE RUN", ru: "ПЛАН ДО ЗАПУСКА" })} ↗</a><a class="quiet-link" href="/experiment/e007/gate-13b/">${localized({ en: "FIRST DEBERTA PASS", ru: "ПЕРВЫЙ ПРОХОД DEBERTA" })} ↗</a></div></section>`;
  } catch (error) {
    target.querySelector(".experiment-loading").textContent = localized({ en: "Gate 13D could not be loaded.", ru: "Не удалось загрузить Gate 13D." });
  }
}
...
async function loadE007AnswerSynthesis() {
  ...
  if (!target) return;
  try {
    const [worldResponse, resultResponse, auditResponse, largerResultResponse, largerAuditResponse] = await Promise.all([
      ...
      fetch("/experiments/E007/answer-synthesis-qwen17b-result-v0.1.json", { cache: "no-store" }),
      fetch("/experiments/E007/answer-synthesis-qwen17b-human-audit-v0.1.json", { cache: "no-store" }),
    ]);
    if (!worldResponse.ok || !resultResponse.ok || !auditResponse.ok || !largerResultResponse.ok || !largerAuditResponse.ok) throw new Error("Gate 14A data missing");
    const world = await worldResponse.json();
    const result = await resultResponse.json();
    const audit = await auditResponse.json();
    const largerResult = await largerResultResponse.json();
    const largerAudit = await largerAuditResponse.json();
    const resultById = new Map(result.records.map(item => [item.id, item]));
    const auditById = new Map(audit.records.map(item => [item.id, item]));
    const largerResultById = new Map(largerResult.records.map(item => [item.id, item]));
    const largerAuditById = new Map(largerAudit.records.map(item => [item.id, item]));
    const cases = world.cases.map(item => {
      const model = resultById.get(item.id);
      const review = auditById.get(item.id);
      const largerModel = largerResultById.get(item.id);
      const largerReview = largerAuditById.get(item.id);
      ...
      const verdict = largerReview.verdict === "pass" ? localized({ en: "1.7B CORRECT", ru: "1.7B ВЕРНО" }) : localized({ en: "1.7B FAILED", ru: "1.7B ОШИБКА" });
      const answerColumn = (name, answer, reviewRecord) => `<section class="e007-synthesis-answer ${reviewRecord.verdict === "pass" ? "is-correct" : "is-critical"}"><small>${escapeHTML(name)} · ${reviewRecord.verdict === "pass" ? localized({ en: "CORRECT", ru: "ВЕРНО" }) : localized({ en: "FAILED", ru: "ОШИБКА" })}</small><p>${escapeHTML(answer)}</p><strong>${escapeHTML(reviewRecord.reason)}</strong></section>`;
      return `<details class="e007-synthesis-case ${largerReview.verdict === "pass" ? "is-correct" : "is-critical"}" open><summary><b>${escapeHTML(item.id)}</b><span>${escapeHTML(item.question)}</span><strong>${escapeHTML(verdict)}</strong></summary><div class="e007-synthesis-input">${piles}</div><div class="e007-synthesis-comparison">${answerColumn("QWEN3-0.6B", model.answer, review)}${answerColumn("QWEN3-1.7B", largerModel.answer, largerReview)}</div></details>`;
    }).join("");
    target.querySelector(".experiment-loading").outerHTML = `<section class="e007-cluster-result"><div class="e005-gate4-result-verdict is-passed"><span>${localized({ en: "LOCKED PAIRED SYNTHETIC DEVELOPMENT RESULT", ru: "ЗАМОРОЖЕННЫЙ ПАРНЫЙ SYNTHETIC DEVELOPMENT-РЕЗУЛЬТАТ" })}</span><h2>${localized({ en: "1.7B kept all ten. 0.6B kept eight.", ru: "1.7B сохранила все десять. 0.6B — восемь." })}</h2><p>${localized({ en: "The larger model repaired both observed failures without inventing a new fact. The same prompt and ten frozen inputs were used.", ru: "Большая модель исправила оба замеченных сбоя и не придумала нового факта. Prompt и десять замороженных входов были теми же." })}</p></div><div class="e007-result-metrics"><article class="is-critical"><span>QWEN3-0.6B</span><strong>${audit.summary.passed_cases} / ${audit.summary.total_cases}</strong></article><article class="is-correct"><span>QWEN3-1.7B</span><strong>${largerAudit.summary.passed_cases} / ${largerAudit.summary.total_cases}</strong></article><article class="is-correct"><span>${localized({ en: "INVENTED FACTS · 1.7B", ru: "ПРИДУМАННЫХ ФАКТОВ · 1.7B" })}</span><strong>${largerAudit.summary.new_factual_claims}</strong></article></div><div class="e007-synthesis-list">${cases}</div><div class="actions"><a class="primary-link" href="/experiments/E007/answer-synthesis-qwen17b-human-audit-v0.1.json">${localized({ en: "1.7B MANUAL AUDIT", ru: "РУЧНАЯ ПРОВЕРКА 1.7B" })} ↗</a><a class="quiet-link" href="/experiments/E007/answer-synthesis-qwen17b-result-v0.1.json">${localized({ en: "1.7B RAW RESULT", ru: "СЫРОЙ РЕЗУЛЬТАТ 1.7B" })} ↗</a><a class="quiet-link" href="/experiments/E007/answer-synthesis-qwen17b-protocol-v0.1.json">${localized({ en: "PAIRED PLAN", ru: "ПАРНЫЙ ПЛАН" })} ↗</a><a class="quiet-link" href="/experiments/E007/answer-synthesis-result-v0.1.json">0.6B ${localized({ en: "RAW RESULT", ru: "СЫРОЙ РЕЗУЛЬТАТ" })} ↗</a><a class="quiet-link" href="/experiments/E007/answer-synthesis-world-v0.1.json">${localized({ en: "ALL TEN INPUTS", ru: "ВСЕ ДЕСЯТЬ ВХОДОВ" })} ↗</a></div></section>`;
  } catch (error) {
...
function render() {
  ...
  } else if (path === "experiment/e007/gate-13b") {
  ...
  } else if (path === "experiment/e007/gate-13d") {
  ...
  } else if (path === "experiment/e007/gate-15f") {
