...
async function loadE005Gate5B2() {
  ...
  if (!target) return;
  try {
    const [response, preflightResponse, calibrationResponse] = await Promise.all([
    ...
    if (!response.ok || !preflightResponse.ok || !calibrationResponse.ok) throw new Error("E005 Gate 5B.2 checkpoint unavailable");
    const data = await response.json();
    const preflight = await preflightResponse.json();
