...
async function loadE005Gate5B2() {
  ...
  if (!target) return;
  try {
    const [response, correctedResponse, preflightResponse, calibrationResponse] = await Promise.all([
    ...
    if (!response.ok || !correctedResponse.ok || !preflightResponse.ok || !calibrationResponse.ok) throw new Error("E005 Gate 5B.2 checkpoint unavailable");
    const data = await response.json();
    ...
    const calibrationMarkup = `<section class="e005-gate4-result-verdict is-failed"><span>${localized({ en: "CALIBRATION v0.1 STOPPED", ru: "КАЛИБРОВКА v0.1 ОСТАНОВЛЕНА" })}</span><h2>J1 · ${calibration.score} / ${calibration.required}</h2><p>${pick(calibration.diagnosis)}</p></section><section class="e005-gate4-lessons-status"><strong>${localized({ en: "ZERO EXPERIMENT ANSWERS SEEN", ru: "УВИДЕНО 0 ОТВЕТОВ ЭКСПЕРИМЕНТА" })}</strong><p>${pick(calibration.decision)}</p></section><section class="e005-gate4-result-verdict"><span>${localized({ en: "RULER v0.2 LOCKED", ru: "ЛИНЕЙКА v0.2 ЗАМОРОЖЕНА" })}</span><h2>${localized({ en: "The judges choose two component labels. Code computes the result.", ru: "Судьи выбирают две метки. Итог вычисляет код." })}</h2><p>${pick(corrected.reason_for_revision)}</p><small>${pick(corrected.claim_boundary)}</small></section>`;
