...
async function loadE005Gate5B2() {
  ...
  if (!target) return;
  try {
    const [response, correctedResponse, rubricV3Response, rubricV4Response, preflightResponse, calibrationResponse, calibrationV2Response, calibrationV3Response, phiLoadResponse] = await Promise.all([
    ...
    if (!response.ok || !correctedResponse.ok || !rubricV3Response.ok || !rubricV4Response.ok || !preflightResponse.ok || !calibrationResponse.ok || !calibrationV2Response.ok || !calibrationV3Response.ok || !phiLoadResponse.ok) throw new Error("E005 Gate 5B.2 checkpoint unavailable");
    const data = await response.json();
    ...
    const calibrationMarkup = `<section class="e005-gate4-result-verdict"><span>${localized({ en: "NEW JUDGES LOCKED BEFORE THE TEST", ru: "НОВЫЕ СУДЬИ ЗАМОРОЖЕНЫ ДО ТЕСТА" })}</span><h2>${localized({ en: "Judge A · 8B + Judge B · 14B", ru: "Судья A · 8B + Судья B · 14B" })}</h2><p>${localized({ en: rubricV4.blindness, ru: "Оба получают только вопрос, ожидаемые смыслы и один ответ. Они не знают модель и вариант, создавшие ответ." })}</p><small>${pick(rubricV4.limitation)}</small></section><section class="e005-gate4-lessons-status"><strong>${localized({ en: "ZERO EXPERIMENT ANSWERS SEEN", ru: "УВИДЕНО 0 ОТВЕТОВ ЭКСПЕРИМЕНТА" })}</strong><p>${localized({ en: "Each new judge must first solve all 12 new traps.", ru: "Каждый новый судья сначала должен решить все 12 новых ловушек." })}</p></section><details class="e005-all-controls"><summary>${localized({ en: "WHY THE OLD JUDGES WERE REJECTED", ru: "ПОЧЕМУ СТАРЫЕ СУДЬИ ОТКЛОНЕНЫ" })}</summary><p>J1 v0.3 · ${calibrationV3.score}/${calibrationV3.required}</p><p>J1 v0.2 · ${calibrationV2.score}/${calibrationV2.required}</p><p>J1 v0.1 · ${calibration.score}/${calibration.required}</p><p>Phi · ${escapeHTML(phiLoad.status)}</p></details>`;
