  ...
  try {
    ...
    if (localOfferResult) {
      ...
      const tableGroups = localOfferResult.questions_review.map((question) => {
        ...
        return `<tbody>${cards.map(([card, item], index) => `<tr class="${item.correct ? "is-correct" : "is-wrong"}">${index === 0 ? `<th rowspan="${cards.length}"><span>${escapeHTML(question.id)}</span>${escapeHTML(question.question)}</th>` : ""}<td><b>${escapeHTML(card)}</b></td><td><span class="e007-state">${escapeHTML(stateNames[item.expected])}</span></td><td><span class="e007-state">${escapeHTML(stateNames[item.actual])}</span></td><td>${item.actual === "empty" ? "—" : escapeHTML(item.source_title)}</td><td class="e007-verdict">${item.correct ? `✓ ${localized({ en: "RIGHT", ru: "ВЕРНО" })}` : `× ${localized({ en: "WRONG", ru: "ОШИБКА" })}`}</td></tr>`).join("")}</tbody>`;
      }).join("");
      const neural = localOfferResult.lane_summaries.multilingual_neural;
      localOfferResultsMarkup = `<section id="e007-local-offer-results" class="e007-local-result-wide"><div class="flow-step">CHECKPOINT 3B · ${localized({ en: "RESULT", ru: "РЕЗУЛЬТАТ" })}</div><h2>${localized({ en: "Meaning search was right 20 times out of 24.", ru: "Поиск по смыслу принял 20 из 24 решений правильно." })}</h2><div class="e007-result-metrics"><article class="is-correct"><span>${localized({ en: "RIGHT DECISIONS", ru: "ВЕРНЫЕ РЕШЕНИЯ" })}</span><strong>${neural.correct_states} / 24</strong></article><article class="is-wrong"><span>${localized({ en: "USEFUL SOURCES", ru: "ПОЛЕЗНЫЕ ИСТОЧНИКИ" })}</span><strong>${neural.required_source_recall.found} / 5</strong></article><article class="is-correct"><span>${localized({ en: "SECRET LEAKS", ru: "УТЕЧКИ СЕКРЕТА" })}</span><strong>0</strong></article></div><p class="e007-baseline-note">${localized({ en: "For comparison: exact words made 5/24 right decisions; text fragments also made 5/24. They said “found” almost every time.", ru: "Для сравнения: точные слова дали 5/24 верных решений; фрагменты текста — тоже 5/24. Они почти всегда говорили «нашла»." })}</p><div class="e007-result-legend"><span class="is-correct">■ ${localized({ en: "green = right", ru: "зелёное = верно" })}</span><span class="is-wrong">■ ${localized({ en: "red = wrong", ru: "красное = ошибка" })}</span></div><div class="e007-result-table-wrap"><table class="e007-result-table"><thead><tr><th>${localized({ en: "QUESTION", ru: "ВОПРОС" })}</th><th>POCKET I</th><th>${localized({ en: "SHOULD DO", ru: "ДОЛЖНА БЫЛА" })}</th><th>${localized({ en: "DID", ru: "СДЕЛАЛА" })}</th><th>${localized({ en: "SELECTED RECORD", ru: "ВЫБРАННАЯ ЗАПИСЬ" })}</th><th>${localized({ en: "RESULT", ru: "ИТОГ" })}</th></tr></thead>${tableGroups}</table></div><p class="control-warning">${localized({ en: "Conclusion: meaning search rejects noise much better, but it still missed one useful hive record. We need one method that does both jobs at once.", ru: "Вывод: поиск по смыслу гораздо лучше отбрасывает мусор, но всё ещё пропустил одну полезную запись про улей. Нужен один метод, который хорошо делает обе вещи сразу." })}</p><div class="actions"><a class="quiet-link" href="/experiments/E007/local-offer-result-L0001.json">${localized({ en: "TECHNICAL DATA", ru: "ТЕХНИЧЕСКИЕ ДАННЫЕ" })} ↗</a></div></section>`;
    }
    target.querySelector(".experiment-loading").outerHTML = `
