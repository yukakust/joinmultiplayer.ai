...
async function loadE005Gate5B2() {
  ...
  if (!target) return;
  try {
    const [response, correctedResponse, rubricV3Response, preflightResponse, calibrationResponse, calibrationV2Response] = await Promise.all([
    ...
    if (!response.ok || !correctedResponse.ok || !rubricV3Response.ok || !preflightResponse.ok || !calibrationResponse.ok || !calibrationV2Response.ok) throw new Error("E005 Gate 5B.2 checkpoint unavailable");
    const data = await response.json();
    ...
    const calibrationMarkup = `<section class="e005-gate4-result-verdict"><span>${localized({ en: "RULER v0.3 LOCKED BEFORE THE TEST", ru: "ЛИНЕЙКА v0.3 ЗАМОРОЖЕНА ДО ТЕСТА" })}</span><h2>${localized({ en: "No claim → absent. Conflict → wrong. Same meaning → correct.", ru: "Не сказал → нет ответа. Спорит → неверно. Смысл тот же → верно." })}</h2><p>${pick(rubricV3.reason_for_revision)}</p><small>${pick(rubricV3.claim_boundary)}</small></section><section class="e005-gate4-result-verdict is-failed"><span>${localized({ en: "PREVIOUS CALIBRATION v0.2 STOPPED", ru: "ПРЕДЫДУЩАЯ КАЛИБРОВКА v0.2 ОСТАНОВЛЕНА" })}</span><h2>J1 · ${calibrationV2.score} / ${calibrationV2.required}</h2><p>${pick(calibrationV2.diagnosis)}</p></section><section class="e005-gate4-lessons-status"><strong>${localized({ en: "ZERO EXPERIMENT ANSWERS SEEN", ru: "УВИДЕНО 0 ОТВЕТОВ ЭКСПЕРИМЕНТА" })}</strong><p>${pick(calibrationV2.decision)}</p></section><details class="e005-all-controls"><summary>${localized({ en: "EARLIER v0.1 · 8/12", ru: "ЕЩЁ РАНЬШЕ v0.1 · 8/12" })}</summary><p>${pick(calibration.diagnosis)}</p></details>`;
