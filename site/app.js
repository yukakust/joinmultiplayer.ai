...
async function loadE005Gate5B2() {
  ...
  if (!target) return;
  try {
    const [response, correctedResponse, rubricV3Response, rubricV4Response, preflightResponse, calibrationResponse, calibrationV2Response, calibrationV3Response, phiLoadResponse, qwen8ResultResponse, qwen14ResultResponse] = await Promise.all([
    ...
    if (!response.ok || !correctedResponse.ok || !rubricV3Response.ok || !rubricV4Response.ok || !preflightResponse.ok || !calibrationResponse.ok || !calibrationV2Response.ok || !calibrationV3Response.ok || !phiLoadResponse.ok || !qwen8ResultResponse.ok || !qwen14ResultResponse.ok) throw new Error("E005 Gate 5B.2 checkpoint unavailable");
    const data = await response.json();
    ...
    const judges = rubricV4.judges.map(judge => `<article><span>${escapeHTML(judge.public_alias)}</span><h2>${escapeHTML(judge.judge_id === "J1" ? "8B" : "14B")}</h2><p>${localized({ en: "The judge does not know who produced the answer.", ru: "Судья не знает, кто создал проверяемый ответ." })}</p><small>${escapeHTML(judge.precision)} · temperature 0 · ${escapeHTML(judge.revision.slice(0, 8))}</small></article>`).join("");
    const calibrationMarkup = `<section class="e005-gate4-result-verdict"><span>${localized({ en: "JUDGE B · 14B PASSED", ru: "СУДЬЯ B · 14B ПРОШЁЛ" })}</span><h2>${qwen14Result.score} / ${qwen14Result.required}</h2><p>${pick(qwen14Result.plain_result)}</p></section><section class="e005-gate4-result-verdict is-failed"><span>${localized({ en: "JUDGE A · 8B REJECTED", ru: "СУДЬЯ A · 8B ОТКЛОНЁН" })}</span><h2>${qwen8Result.score} / ${qwen8Result.required}</h2><p>${pick(qwen8Result.plain_result)}</p></section><section class="e005-gate4-lessons-status"><strong>${localized({ en: "ZERO EXPERIMENT ANSWERS SEEN", ru: "УВИДЕНО 0 ОТВЕТОВ ЭКСПЕРИМЕНТА" })}</strong><p>${localized({ en: "14B may now run blind. A replacement second judge is still required.", ru: "14B теперь может начать слепую проверку. Второго успешного судью ещё нужно заменить." })}</p></section><details class="e005-all-controls"><summary>${localized({ en: "WHY THE OLDER JUDGES WERE REJECTED", ru: "ПОЧЕМУ БОЛЕЕ СТАРЫЕ СУДЬИ ОТКЛОНЕНЫ" })}</summary><p>J1 v0.3 · ${calibrationV3.score}/${calibrationV3.required}</p><p>J1 v0.2 · ${calibrationV2.score}/${calibrationV2.required}</p><p>J1 v0.1 · ${calibration.score}/${calibration.required}</p><p>Phi · ${escapeHTML(phiLoad.status)}</p></details>`;
