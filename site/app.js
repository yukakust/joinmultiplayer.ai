...
async function loadE005Gate5B2() {
  ...
  if (!target) return;
  try {
    const [response, correctedResponse, preflightResponse, calibrationResponse, calibrationV2Response] = await Promise.all([
    ...
    if (!response.ok || !correctedResponse.ok || !preflightResponse.ok || !calibrationResponse.ok || !calibrationV2Response.ok) throw new Error("E005 Gate 5B.2 checkpoint unavailable");
    const data = await response.json();
    ...
    const calibrationMarkup = `<section class="e005-gate4-result-verdict is-failed"><span>${localized({ en: "CALIBRATION v0.2 STOPPED", ru: "КАЛИБРОВКА v0.2 ОСТАНОВЛЕНА" })}</span><h2>J1 · ${calibrationV2.score} / ${calibrationV2.required}</h2><p>${pick(calibrationV2.diagnosis)}</p></section><section class="e005-gate4-lessons-status"><strong>${localized({ en: "ZERO EXPERIMENT ANSWERS SEEN", ru: "УВИДЕНО 0 ОТВЕТОВ ЭКСПЕРИМЕНТА" })}</strong><p>${pick(calibrationV2.decision)}</p></section><details class="e005-all-controls"><summary>${localized({ en: "PREVIOUS v0.1 · 8/12", ru: "ПРЕДЫДУЩАЯ v0.1 · 8/12" })}</summary><p>${pick(calibration.diagnosis)}</p></details><section class="e005-gate4-result-verdict"><span>${localized({ en: "RULER v0.2 WAS LOCKED", ru: "ЛИНЕЙКА v0.2 БЫЛА ЗАМОРОЖЕНА" })}</span><h2>${localized({ en: "The judges choose two component labels. Code computes the result.", ru: "Судьи выбирают две метки. Итог вычисляет код." })}</h2><p>${pick(corrected.reason_for_revision)}</p><small>${pick(corrected.claim_boundary)}</small></section>`;
