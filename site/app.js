...
async function loadE007TenButtons() {
  ...
  if (!target) return;
  try {
    const [protocolResponse, resultResponse, sanityProtocolResponse, sanityResultResponse, ladderProtocolResponse, ladderResultResponse, ninetyProtocolResponse, ninetyResultResponse, reversedProtocolResponse, reversedResultResponse] = await Promise.all([
    ...
    if (!protocolResponse.ok || !resultResponse.ok || !sanityProtocolResponse.ok || !sanityResultResponse.ok || !ladderProtocolResponse.ok || !ladderResultResponse.ok || !ninetyProtocolResponse.ok || !ninetyResultResponse.ok || !reversedProtocolResponse.ok || !reversedResultResponse.ok) throw new Error("compact result unavailable");
    const protocol = await protocolResponse.json();
    const result = await resultResponse.json();
