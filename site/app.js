...
async function loadE005Gate5B2() {
  ...
  if (!target) return;
  try {
    const [response, correctedResponse, rubricV3Response, preflightResponse, calibrationResponse, calibrationV2Response, calibrationV3Response, phiLoadResponse] = await Promise.all([
    ...
    if (!response.ok || !correctedResponse.ok || !rubricV3Response.ok || !preflightResponse.ok || !calibrationResponse.ok || !calibrationV2Response.ok || !calibrationV3Response.ok || !phiLoadResponse.ok) throw new Error("E005 Gate 5B.2 checkpoint unavailable");
    const data = await response.json();
    ...
    const calibrationMarkup = `<section class="e005-gate4-result-verdict is-failed"><span>${localized({ en: "BOTH AUTOMATIC JUDGES REJECTED", ru: "ОБА АВТОМАТИЧЕСКИХ СУДЬИ ОТКЛОНЕНЫ" })}</span><h2>${localized({ en: "192 answers remain unscored", ru: "192 ответа остаются без смысловой оценки" })}</h2><p>${localized({ en: "Mistral failed the meaning test. Phi failed the required evidence form. Neither is allowed to judge E005.", ru: "Mistral не прошёл проверку смысла. Phi не заполнил обязательное доказательство. Ни одному нельзя судить E005." })}</p></section><section class="e005-gate4-lessons-status"><strong>${localized({ en: "ZERO EXPERIMENT ANSWERS SEEN", ru: "УВИДЕНО 0 ОТВЕТОВ ЭКСПЕРИМЕНТА" })}</strong><p>${localized({ en: "The stop rule worked: a bad ruler did not rewrite the experiment result.", ru: "Стоп-правило сработало: плохая линейка не переписала результат эксперимента." })}</p></section><section class="e005-gate4-result-verdict"><span>${localized({ en: "RULER v0.3 WAS LOCKED FIRST", ru: "ЛИНЕЙКА v0.3 БЫЛА ЗАМОРОЖЕНА ЗАРАНЕЕ" })}</span><h2>${localized({ en: "No claim → absent. Conflict → wrong. Same meaning → correct.", ru: "Не сказал → нет ответа. Спорит → неверно. Смысл тот же → верно." })}</h2><small>${pick(rubricV3.claim_boundary)}</small></section><details class="e005-all-controls"><summary>${localized({ en: "ALL CALIBRATION FAILURES", ru: "ВСЕ ПРОВАЛЫ КАЛИБРОВКИ" })}</summary><p>J1 v0.3 · ${calibrationV3.score}/${calibrationV3.required}</p><p>J1 v0.2 · ${calibrationV2.score}/${calibrationV2.required}</p><p>J1 v0.1 · ${calibration.score}/${calibration.required}</p><p>J2 · ${escapeHTML(phiLoad.status)}</p></details>`;
