...
async function loadE007() {
  ...
  try {
    ...
    if (localOfferResult) {
      ...
      const stateNames = {
        found: localized({ en: "found a useful record", ru: "нашла полезную запись" }),
        empty: localized({ en: "found nothing useful", ru: "не нашла ничего полезного" }),
        blocked: localized({ en: "found it but refused to share", ru: "нашла, но отказалась раскрывать" }),
      };
      const laneCards = Object.entries(localOfferResult.lane_summaries).map(([lane, summary]) => `<article class="${lane === localOfferResult.best_lane ? "is-correct" : "is-wrong"}"><span>${escapeHTML(laneNames[lane])}</span><strong>${summary.correct_states} / ${summary.total_states}</strong><p>${localized({ en: "right decisions", ru: "правильных решений" })}</p><small>${localized({ en: "useful sources found", ru: "полезных источников найдено" })}: ${summary.required_source_recall.found}/5<br>${localized({ en: "spoke when it should not", ru: "зря сказала «нашла»" })}: ${summary.false_found}</small><details><summary>${localized({ en: "Technical score", ru: "Техническая оценка" })}</summary><small>macro-F1 ${Number(summary.macro_f1).toFixed(3)}</small></details></article>`).join("");
      const questionRows = localOfferResult.questions_review.map((question, index) => {
        const methods = ["multilingual_neural", "exact_terms", "chargram_vector"].map((lane) => {
          const cards = question.methods[lane];
          const cardRows = Object.entries(cards).map(([card, item]) => {
            const resultText = item.correct
              ? `${localized({ en: "As expected", ru: "Как и ожидали" })}: ${stateNames[item.actual]}`
              : `${localized({ en: "Expected", ru: "Ожидали" })}: ${stateNames[item.expected]}. ${localized({ en: "Actually", ru: "На деле" })}: ${stateNames[item.actual]}`;
            const source = item.actual === "empty" ? "" : `<p>${localized({ en: "Selected record", ru: "Выбранная запись" })}: <b>${escapeHTML(item.source_title)}</b></p>`;
            return `<li class="${item.correct ? "is-correct" : "is-wrong"}"><b>${item.correct ? "✓" : "×"} ${escapeHTML(card)}</b><span>${escapeHTML(resultText)}</span>${source}<details><summary>${localized({ en: "Technical details", ru: "Технические детали" })}</summary><small>${escapeHTML(item.source_id)} · score ${Number(item.score).toFixed(3)}</small></details></li>`;
          }).join("");
      ...
      const howToRead = `<section class="e007-smoke-lock"><h2>${localized({ en: "How to read this test", ru: "Как читать этот тест" })}</h2><p>${localized({ en: "We asked 6 questions to 4 separate pocket i. That makes 24 decisions for each search method. Before the run, we wrote down what each pocket i should do.", ru: "Мы задали 6 вопросов четырём отдельным pocket i. Получилось 24 решения для каждого способа поиска. До запуска мы записали, как должна поступить каждая pocket i." })}</p><div class="e007-answer-grid"><article><span>FOUND</span><strong>${localized({ en: "I have something useful", ru: "У меня есть полезное" })}</strong></article><article><span>EMPTY</span><strong>${localized({ en: "I have nothing useful", ru: "Мне нечего добавить" })}</strong></article><article><span>BLOCKED</span><strong>${localized({ en: "I found it, but it is private", ru: "Нашла, но это приватное" })}</strong></article></div><p>${localized({ en: "Green means the pocket i behaved as expected. Red means it spoke when it should stay silent, stayed silent when it had useful knowledge, or broke the privacy rule.", ru: "Зелёное означает: pocket i поступила как ожидалось. Красное: заговорила зря, промолчала при полезном знании или нарушила правило приватности." })}</p></section>`;
      localOfferResultsMarkup = `<section id="e007-local-offer-results" class="e007-smoke-results"><div class="flow-step">CHECKPOINT 3B · ${localized({ en: "TWO DEVICES FINISHED", ru: "ДВА УСТРОЙСТВА ЗАКОНЧИЛИ" })}</div><h2>${localized({ en: "Meaning search made 20 of 24 decisions correctly—but missed one useful record.", ru: "Поиск по смыслу принял 20 из 24 решений правильно — но пропустил одну полезную запись." })}</h2>${howToRead}<div class="e005-metrics"><article><span>${localized({ en: "DEVICES", ru: "УСТРОЙСТВА" })}</span><strong>2</strong></article><article><span>POCKET I</span><strong>4</strong></article><article><span>${localized({ en: "DECISIONS", ru: "РЕШЕНИЯ" })}</span><strong>24 / 24</strong></article></div><h2>${localized({ en: "Three ways to search the same memory", ru: "Три способа искать в одной и той же памяти" })}</h2><div class="e007-answer-grid">${laneCards}</div><p class="control-warning">${escapeHTML(localized(localOfferResult.protocol_design_finding))}</p><details class="e005-all-controls"><summary>${localized({ en: "Technical success gates", ru: "Технические ворота успеха" })}</summary><div class="e007-gates"><ul>${gates}</ul></div></details><h2>${localized({ en: "Open a question and compare what happened", ru: "Откройте вопрос и сравните, что произошло" })}</h2><div class="e005-tasks">${questionRows}</div><p class="control-warning">${escapeHTML(localOfferResult.claim_boundary)}</p><div class="actions"><a class="quiet-link" href="/experiments/E007/local-offer-result-L0001.json">SCORED RESULT JSON ↗</a><a class="quiet-link" href="/api/public/L0001">ALL RAW RECEIPTS JSON ↗</a></div></section>`;
    }
    target.querySelector(".experiment-loading").outerHTML = `
      ${localOfferMarkup}
