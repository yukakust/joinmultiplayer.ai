...
async function loadE007AnswerPilesQwenSandwich() {
  ...
  try {
    ...
    if (!resultResponse.ok || !auditResponse.ok) throw new Error("Gate 13D data missing");
    const result = await resultResponse.json();
    const audit = await auditResponse.json();
    ...
    const findingText = {
      ...
      "AUDIT-02": localized({ en: "Unsafe pass: Qwen dropped the word ‘exclusively’, but the guard accepted it.", ru: "Опасный пропуск: Qwen потеряла слово «только», но защита всё равно приняла фразу." }),
    ...
    const findings = audit.findings.map(finding => `<article class="e007-audit-card ${finding.verdict === "correctly_blocked" ? "is-correct" : "is-critical"}"><b>${escapeHTML(finding.id)}</b><strong>${escapeHTML(findingText[finding.id])}</strong><p>${escapeHTML(finding.reason)}</p></article>`).join("");
    target.querySelector(".experiment-loading").outerHTML = `<section class="e007-cluster-result"><div class="e005-gate4-result-verdict is-failed"><span>${localized({ en: "LOCKED PAIRED DEVELOPMENT RESULT", ru: "ЗАМОРОЖЕННЫЙ ПАРНЫЙ DEVELOPMENT-РЕЗУЛЬТАТ" })}</span><h2>${localized({ en: "Better score, but the safety check leaked.", ru: "Стало лучше, но защита дала течь." })}</h2><p>${localized({ en: "The sandwich improved 4/6 piles to 5/6 without a false merge, but one meaning-changing Qwen rewrite was accepted.", ru: "Сэндвич улучшил результат с 4/6 до 5/6 без ложной склейки, но одна изменившая смысл фраза Qwen прошла проверку." })}</p></div><div class="e007-result-metrics"><article class="is-correct"><span>${localized({ en: "FINAL CORRECT PILES", ru: "ПРАВИЛЬНЫХ СТОПОК" })}</span><strong>${result.summary.final_exact_paraphrase_piles} / 6</strong></article><article class="is-correct"><span>${localized({ en: "FALSE MERGES", ru: "ЛОЖНЫХ СКЛЕЕК" })}</span><strong>${result.summary.false_merges}</strong></article><article class="is-critical"><span>${localized({ en: "SAFE REWRITES", ru: "ПРОВЕРЕННЫХ ПЕРЕПИСЫВАНИЙ" })}</span><strong>${result.summary.valid_canonical_claims} / 10</strong></article></div><div class="e007-audit-list">${findings}</div><h2>${localized({ en: "All ten Qwen rewrites", ru: "Все десять переписываний Qwen" })}</h2><div class="e007-canonical-list">${canonicalCards}</div><div class="actions"><a class="quiet-link" href="/experiments/E007/answer-piles-qwen-sandwich-result-v0.1.json">${localized({ en: "ALL CHECKS", ru: "ВСЕ ПРОВЕРКИ" })} ↗</a><a class="quiet-link" href="/experiments/E007/answer-piles-qwen-sandwich-human-audit-v0.1.json">${localized({ en: "MANUAL AUDIT", ru: "РУЧНАЯ ПРОВЕРКА" })} ↗</a><a class="quiet-link" href="/experiments/E007/answer-piles-qwen-sandwich-invalid-preflight-v0.1.json">INVALID PREFLIGHT ↗</a><a class="quiet-link" href="/experiments/E007/answer-piles-qwen-sandwich-protocol-v0.1.json">${localized({ en: "PLAN BEFORE RUN", ru: "ПЛАН ДО ЗАПУСКА" })} ↗</a><a class="quiet-link" href="/experiment/e007/gate-13b/">${localized({ en: "FIRST DEBERTA PASS", ru: "ПЕРВЫЙ ПРОХОД DEBERTA" })} ↗</a></div></section>`;
  } catch (error) {
