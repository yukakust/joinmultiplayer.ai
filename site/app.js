...
async function loadE005Gate5B() {
  ...
  if (!target) return;
  try {
    const [designResponse, curriculumResponse, examResponse, preflightResponse, smokeResponse] = await Promise.all([
    ...
    if (!designResponse.ok || !curriculumResponse.ok || !examResponse.ok || !preflightResponse.ok || !smokeResponse.ok) throw new Error("E005 Gate 5B checkpoint unavailable");
    const design = await designResponse.json();
