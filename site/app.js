...
async function loadE005Gate5B() {
  ...
  if (!target) return;
  try {
    const [designResponse, curriculumResponse, examResponse, preflightResponse, smokeResponse, trainingResponse, mergerSmokeResponse] = await Promise.all([
    ...
    if (!designResponse.ok || !curriculumResponse.ok || !examResponse.ok || !preflightResponse.ok || !smokeResponse.ok || !trainingResponse.ok || !mergerSmokeResponse.ok) throw new Error("E005 Gate 5B checkpoint unavailable");
    const design = await designResponse.json();
    ...
    target.querySelector(".e005-gate4-lessons-status strong").textContent = localized({ en: "BOTH PERSONAL TRACKS TRAINED · LOCKED EXAM NEVER RUN", ru: "ОБА ЛИЧНЫХ ТРЕКА ОБУЧЕНЫ · ЭКЗАМЕН НЕ ЗАПУСКАЛСЯ" });
