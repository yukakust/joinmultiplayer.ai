...
function e005Gate5BResultsShell() {
  return `<section class="flow-shell e005-gate5b-results-page"><div class="flow-step">E005 · GATE 5B · ${localized({ en: "LOCKED RESULT", ru: "ЗАМОРОЖЕННЫЙ РЕЗУЛЬТАТ" })}</div><h1>${localized({ en: "The first bridge lost one voice.", ru: "Первый мостик потерял один голос." })}</h1><p class="contribution-intro">${localized({ en: "Open every question and compare all six unedited answers.", ru: "Откройте каждый вопрос и сравните все шесть ответов без редактуры." })}</p><div class="experiment-loading">${c("loading")}</div></section>`;
}
...
async function loadE005Gate5BResults() {
  ...
  if (!target) return;
  try {
    const response = await fetch("/experiments/E005/gate-5b-results-v0.1.json", { cache: "no-store" });
    ...
    let index = 0;
    const countCards = conditions.map(condition => `<article><span>${names[condition]}</span><strong>${data.counts[condition]} / 32</strong></article>`).join("");
    target.querySelector(".experiment-loading").outerHTML = `<section class="e005-gate4-result-verdict is-failed"><span>GATE 5B · ${localized({ en: "FAILED", ru: "НЕ ПРОЙДЕН" })}</span><h2>${localized({ en: "The correct pair scored 2 of 32. It needed 26.", ru: "Правильная пара получила 2 из 32. Нужно было 26." })}</h2><p>${localized({ en: "CAUSE-I survived the merge on all questions. SAFETY-I was almost always silenced. The first hidden-state bridge did not combine the two skills.", ru: "CAUSE-I пережил объединение во всех вопросах. SAFETY-I почти всегда оказался заглушён. Первый мостик скрытых состояний не объединил два умения." })}</p></section><div class="e005-metrics">${countCards}</div><p class="control-warning">${localized({ en: "These marks use strict phrase matching and are provisional. Read the raw answers below before accepting them.", ru: "Это предварительные метки по строгому совпадению фраз. Прочитайте сырые ответы ниже, прежде чем им верить." })}</p><div class="e005-gate4c-result-viewer"></div><div class="actions"><a class="button secondary" href="/experiment/e005/gate-5b/">${localized({ en: "BACK TO THE EXPERIMENT", ru: "НАЗАД К ЭКСПЕРИМЕНТУ" })}</a><a class="quiet-link" href="/experiments/E005/gate-5b-results-v0.1.json">ALL 192 RAW ANSWERS JSON ↗</a></div>`;
    const render = () => {
      ...
      const scoreLabel = score => `${score.cause_hit ? "✓" : "×"} ${localized({ en: "cause", ru: "причина" })} · ${score.safety_hit ? "✓" : "×"} ${localized({ en: "safe action", ru: "безопасное действие" })}`;
      target.querySelector(".e005-gate4c-result-viewer").innerHTML = `<div class="e005-gate4-question-nav"><span>${localized({ en: "QUESTION", ru: "ВОПРОС" })} ${index + 1} / ${rows.length}</span><span>${escapeHTML(base.question_id)}</span></div><section class="e005-gate4-current-question"><h2>${escapeHTML(base.question)}</h2><div><span>${localized({ en: "EXPECTED CAUSE", ru: "НУЖНАЯ ПРИЧИНА" })}</span><p>${escapeHTML(base.expected_cause)}</p></div><div><span>${localized({ en: "EXPECTED SAFE ACTION", ru: "НУЖНОЕ БЕЗОПАСНОЕ ДЕЙСТВИЕ" })}</span><p>${escapeHTML(base.expected_safety)}</p></div></section><section class="e005-human-result-focus ${neural.automatic_score.complete ? "is-correct" : "is-wrong"}"><span>${localized({ en: "CORRECT NEURAL PAIR · FINAL ANSWER", ru: "ПРАВИЛЬНАЯ НЕЙРОННАЯ ПАРА · ИТОГОВЫЙ ОТВЕТ" })}</span><h2>${escapeHTML(neural.answer || "—")}</h2><strong>${scoreLabel(neural.automatic_score)}</strong></section><details class="e005-all-controls" open><summary>${localized({ en: "COMPARE ALL SIX ANSWERS", ru: "СРАВНИТЬ ВСЕ ШЕСТЬ ОТВЕТОВ" })}</summary><div class="e005-gate4-result-answers">${conditions.map((condition, order) => { const answer = answers[condition]; const score = answer.automatic_score; const state = score.complete ? "correct" : (score.cause_hit || score.safety_hit ? "partial" : "wrong"); return `<article class="is-${state}"><header><strong>${order + 1} · ${names[condition]}</strong><span>${score.complete ? "●" : (state === "partial" ? "◐" : "×")}</span></header><p>${escapeHTML(answer.answer || "—")}</p><small>${scoreLabel(score)}</small></article>`; }).join("")}</div></details><nav class="e005-gate4-question-controls"><button data-gate5b-previous ${index === 0 ? "disabled" : ""}>←</button><button data-gate5b-next ${index === rows.length - 1 ? "disabled" : ""}>→</button></nav>`;
    };
