...
async function loadE005Gate5C() {
  ...
  if (!target) return;
  try {
    const [response, smokeResponse, correctedResponse] = await Promise.all([
    ...
    if (!response.ok || !smokeResponse.ok || !correctedResponse.ok) throw new Error("E005 Gate 5C design unavailable");
    const data = await response.json();
    const smoke = await smokeResponse.json();
    const corrected = await correctedResponse.json();
    const pick = value => escapeHTML(value?.[language] || value?.en || "");
