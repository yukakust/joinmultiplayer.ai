...
async function loadE005Gate5B2() {
  ...
  if (!target) return;
  try {
    const [response, correctedResponse, rubricV3Response, rubricV4Response, rubricV5Response, preflightResponse, calibrationResponse, calibrationV2Response, calibrationV3Response, phiLoadResponse, qwen8ResultResponse, qwen14ResultResponse, qwen14FullResponse] = await Promise.all([
    ...
    if (!response.ok || !correctedResponse.ok || !rubricV3Response.ok || !rubricV4Response.ok || !rubricV5Response.ok || !preflightResponse.ok || !calibrationResponse.ok || !calibrationV2Response.ok || !calibrationV3Response.ok || !phiLoadResponse.ok || !qwen8ResultResponse.ok || !qwen14ResultResponse.ok || !qwen14FullResponse.ok) throw new Error("E005 Gate 5B.2 checkpoint unavailable");
    const data = await response.json();
    ...
    const judges = `<article><span>${escapeHTML(rubricV5.replacement.public_alias)}</span><h2>14B · 48 ${localized({ en: "layers", ru: "слоёв" })}</h2><p>${localized({ en: "Replacement judge; calibration is still closed.", ru: "Новый судья; входной экзамен ещё закрыт." })}</p><small>BF16 · temperature 0 · ${escapeHTML(rubricV5.replacement.revision.slice(0, 8))}</small></article><article><span>${escapeHTML(rubricV5.existing_judge.public_alias)}</span><h2>14B · 40 ${localized({ en: "layers", ru: "слоёв" })}</h2><p>${localized({ en: "Passed 12/12 and completed 192 blind reviews.", ru: "Прошёл 12/12 и завершил 192 слепые проверки." })}</p><small>BF16 · temperature 0</small></article>`;
