...
async function loadE005Gate5B2() {
  ...
  try {
    ...
    const calibrationMarkup = `<section class="e005-gate4-result-verdict is-failed"><span>${localized({ en: "MISTRAL J1 REJECTED", ru: "СУДЬЯ MISTRAL J1 ОТКЛОНЁН" })}</span><h2>${calibrationV3.score} / ${calibrationV3.required}</h2><p>${pick(calibrationV3.diagnosis)}</p></section><section class="e005-gate4-lessons-status"><strong>${localized({ en: "PHI J2 LOAD RETRY", ru: "ПОВТОР ЗАГРУЗКИ PHI J2" })}</strong><p>${localized({ en: phiLoad.fix, ru: "Те же веса Phi загружаются встроенным кодом Transformers. Линейка и модель не меняются." })}</p></section><section class="e005-gate4-lessons-status"><strong>${localized({ en: "ZERO EXPERIMENT ANSWERS SEEN", ru: "УВИДЕНО 0 ОТВЕТОВ ЭКСПЕРИМЕНТА" })}</strong><p>${pick(calibrationV3.decision)}</p></section><section class="e005-gate4-result-verdict"><span>${localized({ en: "RULER v0.3 WAS LOCKED FIRST", ru: "ЛИНЕЙКА v0.3 БЫЛА ЗАМОРОЖЕНА ЗАРАНЕЕ" })}</span><h2>${localized({ en: "No claim → absent. Conflict → wrong. Same meaning → correct.", ru: "Не сказал → нет ответа. Спорит → неверно. Смысл тот же → верно." })}</h2><small>${pick(rubricV3.claim_boundary)}</small></section><details class="e005-all-controls"><summary>${localized({ en: "PREVIOUS FAILURES · v0.2 9/12 · v0.1 8/12", ru: "ПРЕДЫДУЩИЕ ПРОВАЛЫ · v0.2 9/12 · v0.1 8/12" })}</summary><p>${pick(calibrationV2.diagnosis)}</p><p>${pick(calibration.diagnosis)}</p></details>`;
