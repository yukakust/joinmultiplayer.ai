...
def main() -> None:
    ...
    with sqlite3.connect(args.db) as db:
        ...
        if args.command == "list":
            rows = db.execute(
                "SELECT public_id, door, author, status, created_at FROM contributions ORDER BY row_id DESC LIMIT 100"
            ).fetchall()
            for row in rows:
                print("\t".join(str(row[key]) for key in row.keys()))
        elif args.command == "show":
            row = db.execute(
                "SELECT public_id, door, payload, author, status, review_note, created_at, updated_at FROM contributions WHERE public_id = ?",
                (args.id,),
            ).fetchone()
            if row is None:
