...
BUILDER_PATH = ROOT / "experiments/E007-harness-mvp/src/build_nonce_word_gate.py"
SPEC = importlib.util.spec_from_file_location("build_nonce_word_gate", BUILDER_PATH)
BUILDER = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(BUILDER)


class NonceWordWorldTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.world = json.loads((ROOT / "site/experiments/E007/nonce-word-world-v0.1.json").read_text())

    def test_two_mirrored_decks(self):
        self.assertEqual(self.world["decks"]["MAIN"], {"KSEL": "approve", "PTHY": "reject"})
        self.assertEqual(self.world["decks"]["MIRROR"], {"KSEL": "reject", "PTHY": "approve"})
