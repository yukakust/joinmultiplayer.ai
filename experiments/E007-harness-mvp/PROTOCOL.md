...
## Закон языка экспериментов

Все новые harness-тесты выполняются целиком на английском: вопрос, проверяемое утверждение, тестовый источник, точная цитата, инструкции моделям и человеческая эталонная метка. Перевод внутри измеряемого pipeline запрещён: он добавляет отдельную модель и смешивает ошибку перевода с ошибкой проверяемого модуля. Реальные многоязычные данные будут отдельным будущим Gate после подтверждения English-only MVP.

## Gate 16G.7 — точная цитата + multilingual NLI

Перед запуском зафиксирован следующий turnstile для 26 ответов `FOUND` из Gate 16G.6:

1. Qwen3-8B получает только уже выбранные ею сообщения и либо копирует одну точную цитату, доказывающую всё утверждение, либо отвечает `NO_QUOTE`.
2. Обычный код проверяет ID сообщения, длину цитаты и её посимвольное присутствие в источнике.
3. `mDeBERTa-v3-base-mnli-xnli` сравнивает окно исходного сообщения с русским утверждением.
4. Проходит только `entailment`; `neutral`, `contradiction`, отсутствие цитаты и сломанный формат отклоняются.

Зафиксированный порог успеха: сохранить `9/9` обоснованных кандидатов, пропустить `0/17` необоснованных, принять только точные цитаты и получить `0` сломанных ответов. Это development-тест на уже открытых человеческих метках, а не слепая финальная проверка.

Результат сохранён, но архитектурный вывод отозван: вопросы и claims были русскими, а источники — смесью русского и английского. Это нарушает принятый English-only контракт. Числа остаются наблюдением mixed-language diagnostic (`2/9` полезных сохранено, `2/17` ошибок пропущено), но не являются оценкой English DeBERTa и не могут принять или отклонить модуль MVP.

Отдельно выяснилось, что новый quote-turnstile проверял не ту же способность Qwen, что предыдущий reader. Reader мог указать до четырёх сообщений и пересказать найденный смысл. Quote-turnstile потребовал одну буквальную цитату до 600 символов, доказывающую весь составной claim. В трёх потерянных хороших случаях Qwen: один раз вернула собственный пересказ вместо подстроки, один раз нашла доказательство длиннее искусственного лимита, один раз не нашла единственную цитату для составного claim. Это поломка контракта single-span, а не опровержение способности Qwen находить нужное сообщение.

## Gate 16G.8 — English atomic evidence

Исправленный тест заморожен до inference. В нём 20 полностью английских synthetic cases: 10 подтверждаемых и 10 похожих, но неверных. Qwen3-8B может вернуть от одного до трёх точных spans, которые вместе доказывают весь атомарный claim, либо `NO_EVIDENCE`. Код посимвольно проверяет каждый span. Английская `DeBERTa-v3-base-mnli-fever-anli` отдельно оценивается на заранее зафиксированных oracle spans, после чего измеряется вся связка.

Порог development-успеха: Qwen сохраняет минимум `9/10` хороших и отправляет spans максимум для `1/10` плохих; English DeBERTa сохраняет минимум `9/10` хороших и принимает максимум `1/10` плохих; end-to-end выполняет те же границы. Прохождение разрешит только новый blind English replication.

Результат: locked Gate провален. Qwen вернула точные spans для `10/10` хороших claims, но ручное ревью подтвердило полноту лишь `8/10`: дважды отсутствовал факт, который включает условное правило. Qwen также дважды отправила противоречащие spans для плохого claim и не сломала ни одного tool-call. English DeBERTa на oracle evidence приняла `7/10` хороших и `0/10` плохих. Механический end-to-end принял `7/10`, но после проверки полноты доказательства валидны только `5/10`; плохих принято `0/10`. Вывод: English-only и multi-span исправили буквальное цитирование, но не полноту evidence set; DeBERTa нельзя оставлять единственным судьёй.

## Desktop MVP Checkpoint 0

31 августа 2026 владелец исключил телефон из MVP и принял один desktop-scope:
одинаковый harness для macOS, Windows и Linux. Семь этапов — единый core,
локальная библиотека, поиск доказательств, безопасный обмен, сборка ответа,
простой UI и три установщика с одним свежим locked physical test. Обучение
